/* This class defines a form for entering info about conditions on an action participant for a destory objects rule */
...
{
	private DestroyObjectsRuleParticipantCondition conditionInFocus; // temporary copy of condition whose values are being edited
	...
	private ActionTypeParticipantConstraint constraintInFocus; // particular constraint of the condition being edited
	...
	public DestroyObjectsRuleParticipantConditionInfoForm(JDialog owner, DestroyObjectsRuleParticipantCondition cond)
	{
		super(owner, true);
		actualCondition = cond;
		conditionInFocus = (DestroyObjectsRuleParticipantCondition)(actualCondition.clone());
	...
	public void actionPerformed(ActionEvent evt) // handles user actions
	{
		...
		if(source == okButton) // okButton has been pressed
		{
			...
			if(errors.size() == 0) // input valid
			{
				...
				// set the values to the actual condition from the copy whose values have been edited:
	...
	private void setConstraintInFocus()
	{
		...
		constraintInFocus = conditionInFocus.getConstraint((String)(typeList.getSelectedValue()));
		// set title:
