...
public class CreateObjectsRuleStartingValForm extends JDialog implements
  ...
  public CreateObjectsRuleStartingValForm(JDialog owner,
    ...
    for (int i = 0; i < attributes.size(); i++) {
      ...
      attLabel.append(att.getAttribute().getName() + " ("
          + AttributeTypes.getText(att.getAttribute().getType())); // attribute
                                                                   // name &
      // type
  ...
  private Vector<String> validateInput() { 
    ...
    for (int i = 0; i < values.size(); i++) {
                                                     ...
                                                     // input
      {
        ...
        {
          if (tempAtt.getAttribute().getType() == AttributeTypes.STRING) // string
                                                                         // attribute
          {
          ...
          else if (tempAtt.getAttribute().getType() == AttributeTypes.DOUBLE) // double
                                                                              // attribute
          {
            try {
              ...
              {
                if (doubleVal > ((NumericalAttribute) (tempAtt.getAttribute()))
                    .getMaxValue().doubleValue()) // above the
                // maximum value
                {
                  messages
                      .add(new String(
              ...
              if (tempAtt.getAttribute().isKey()) // key attribute
              {
                if (keyValueIsUnique(new Double(doubleVal)) == false) // key
                                                                      // value
                                                                      // is not
                                                                      // unique
                {
          ...
          else if (tempAtt.getAttribute().getType() == AttributeTypes.INTEGER) // integer
                                                                               // attribute
          {
            try {
              ...
              // value
              {
                if ((intVal < ((NumericalAttribute) (tempAtt.getAttribute()))
                    .getMinValue().intValue())
                    || (intVal > ((NumericalAttribute) (tempAtt.getAttribute()))
                        .getMaxValue().intValue())) // it's outside
                // of the range
                {
                  messages
                      .add(new String(
              ...
              {
                if (intVal < ((NumericalAttribute) (tempAtt.getAttribute()))
                    .getMinValue().intValue()) // below the minimum
                // value
                {
                  messages
                      .add(new String(
              ...
              {
                if (intVal > ((NumericalAttribute) (tempAtt.getAttribute()))
                    .getMaxValue().intValue()) // above the maximum
                // value
                {
                  messages
                      .add(new String(
              ...
              if (tempAtt.getAttribute().isKey()) // key attribute
              {
                if (keyValueIsUnique(new Integer(intVal)) == false) // key value
                                                                    // is not
                                                                    // unique
                {
                  messages
  ...
  private boolean keyValueIsUnique(Object value) // returns true if there is no
                                                 // other object created by this
                                                 // or another rule with the
  // same object type as the object in focus and key attribute value as the one
  // that is being passed into this function
  {
    ...
    // different than its previous value
    {
      ...
      for (int i = 0; i < actionTypes.size(); i++) {
        Vector alreadyCreatedRules = ((ActionType) actionTypes.elementAt(i))
            .getAllCreateObjectsRules(); // get all already
        // created create objects rules of this ActionType
        for (int j = 0; j < alreadyCreatedRules.size(); j++) {
          CreateObjectsRule tempRule = (CreateObjectsRule) alreadyCreatedRules
              .elementAt(j);
          Vector createObjs = tempRule.getAllSimSEObjects(); // get all objects
                                                             // this rule
                                                             // creates
  ...
  private void addEditObject() // sets the attributes of the SimSEObject in
                               // focus from the existing info in the form and
                               // adds it to the
  // rule in focus. Note: validateInput() should be called before calling this
  // function to ensure that you're adding a valid object
  {
    ...
    for (int i = 0; i < values.size(); i++) {
                                                     ...
                                                     // input
      {
                                                                            ...
                                                                            // attribute
        {
          try {
            ...
            // have been called immediately before this to ensure that the input
            // is valid
          }
        }
      ...
      else if (values.elementAt(i) instanceof JComboBox) // combo box (boolean)
                                                         // input
      {
