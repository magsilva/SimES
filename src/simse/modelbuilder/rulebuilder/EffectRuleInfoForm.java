...
public class EffectRuleInfoForm extends JDialog implements ActionListener, KeyListener, MouseListener
{
	...
	private ActionTypeParticipant participantInFocus; // participant currently the focus of this GUI
	private SimSEObjectType objectTypeInFocus; // SimSEObjectType of the participant currently in focus
	...
	private JButton attributeThisPartButton; // attributes of the participant in focus
	private JButton attributeOtherPartButton; // attributes of the other participants in the action
	private JButton numObjectsButton; // numbers of objects in the action
	private JButton numActionsThisPartButton; // number of actions the participant in focus is involved in
	private JButton numActionsOtherPartButton; // number of actions the other participants in the action are involved in
	...
	private Box effectsMiddleMiddlePane; // pane for holding all of the participant info
	private Vector textFields; // Vector of text fields, one for each attribute of the object type currently in focus
	private JList participantList; // list of participants to choose from for editing
	...
	private JPanel otherActEffectPanel; // main panel for the specifying effect on participant's other actions:
	private ButtonGroup buttonGroupOtherActEffect; // for following radio buttons
	private JRadioButton activateButton; // for activating all other actions of participant in focus
	private JRadioButton deactivateButton; // for deactivating all other actions of participant in focus
	private JRadioButton noneButton; // for specifying no effects on the other actions of the participant in focus
	private JList ruleInputList; // list for holding rule inputs
	private JButton newRuleInputButton; // for adding a new rule input
	private JButton viewEditInputButton; // for viewing/editing existing rule input
	...
	public EffectRuleInfoForm(JFrame owner, EffectRule rule, ActionType act, DefinedActionTypes acts,
		DefinedObjectTypes objs, boolean newOrEdit)
	{
		...
		{
			ruleInFocus = (EffectRule)(actualRule.clone()); // make a copy of the rule for editing
		}
		else // new rule
		{
			ruleInFocus = rule;
		...
		effectsMiddlePane.setVerticalScrollBarPolicy(JScrollPane.VERTICAL_SCROLLBAR_ALWAYS);
	...
	public void actionPerformed(ActionEvent evt) // handles user actions
	{
		...
		else if(source == timeElapsedButton)
		{
			...
			if((getLastTokenType(lastFocusedTextField.getText()) == DIGIT) && (getLastToken(lastFocusedTextField.getText()).equals("-"))) // negative
				// sign was last token
			{
				setFocusedTextFieldText(lastFocusedTextField.getText().trim().concat("totalTimeElapsed "));
			}
			else
			{
				setFocusedTextFieldText(lastFocusedTextField.getText().concat("totalTimeElapsed "));
			}
		...
		else if(source == okButton) // okButton has been pressed
		{
			...
			Vector errors = validateInput(); // check validity of input of object type currently in focus
			if(errors.size() == 0) // input valid
			{
				if(!newRule) // existing rule being edited
				{
					// set the values to the actual participant from the copy whose values have been edited:
		...
		else if((source instanceof JButton) && (((JButton)source).getText().length() == 1) &&
			((Character.isDigit(((JButton)source).getText().toCharArray()[0])) || (((JButton)source).getText().toCharArray()[0] == '.')))
				// digit button (or '.' button)
		{
			digitButtonChosen((JButton)source);
		}

		else if((source instanceof JButton) && ((((JButton)source).getText().equals("+")) || (((JButton)source).getText().equals("-"))
			|| (((JButton)source).getText().equals("*")) || (((JButton)source).getText().equals("/")))) // operator button chosen
		{
			operatorButtonChosen((JButton)source);
		}

		else if((source instanceof JButton) && (((JButton)source).getText().equals("(")))
		{
			// append the text to the text field:
			if((getLastTokenType(lastFocusedTextField.getText()) == DIGIT) && (getLastToken(lastFocusedTextField.getText()).equals("-"))) // negative
				// sign was last token
	...
	private void refreshParticipantList()
	{
		...
		{
			ActionTypeParticipant tempPart = (ActionTypeParticipant)parts.elementAt(i);
			partNames.add(tempPart.getName() + ":"); // add each participant's name to the list
			...
			for(int j=0; j<objTypes.size(); j++)
			{
				...
				partNames.add("--" + tempType.getName() + " " + SimSEObjectTypeTypes.getText(tempType.getType()));
			}
	...
	private void setupParticipantListSelectionListenerStuff() // enables view/edit effects button whenever a list item (participant's
		// object type) is selected
	{
	    ...
	    rowSM.addListSelectionListener(new ListSelectionListener(){
	        public void valueChanged(ListSelectionEvent e) {
	            ...
	            if ((lsm.isSelectionEmpty() == false) && (((String)participantList.getSelectedValue()).startsWith("--"))) // a
	            	// object type is selected (not a participant name)
				{
					viewEditEffectsButton.setEnabled(true);
				}
	            else if ((lsm.isSelectionEmpty() == false) && (((String)participantList.getSelectedValue()).startsWith("--")) == false)
	...
	private void viewEditEffects()
	{
		...
		if(selectedIndex >= 0) // a participant is selected
		{
			...
			// move up in the list from the selected item 'til you find the participant name:
			...
			ActionTypeParticipant tempPart = actionInFocus.getParticipant(item.substring(0, (item.length()-1))); // trim off
				// the colon to get the participant name
			SimSEObjectType tempType = tempPart.getSimSEObjectType(selectedString.substring(2, selectedString.indexOf(" ")));
				// trim off dashes at beginning to get the type name
			if((objectTypeInFocus != null) && (((tempPart == participantInFocus) && (tempType != objectTypeInFocus))
				|| (tempPart != participantInFocus))) // selection is not the SimSEObjectType and participant that was already in focus
			{
				setParticipantInFocusDataFromForm();
	...
	private Vector validateInput() // returns a vector of error messages (if any)
	{
		...
		{
			ParticipantRuleEffect tempRuleEffect = (ParticipantRuleEffect)partEffects.elementAt(i);
			...
			{
				ParticipantTypeRuleEffect tempTypeRuleEffect = (ParticipantTypeRuleEffect)partTypeEffects.elementAt(j);
				...
				for(int k=0; k<attRuleEffects.size(); k++)
				{
					...
					ParticipantAttributeRuleEffect tempAttEffect = (ParticipantAttributeRuleEffect)attRuleEffects.elementAt(k);
					if((tempAttEffect.getAttribute().getType() == AttributeTypes.INTEGER)
						|| (tempAttEffect.getAttribute().getType() == AttributeTypes.DOUBLE)) // numerical attribute (needs checking)
					{
						...
						if((effect != null) && (effect.length() > 0)) // non-empty
						{
							...
							if(numOpen > numClose)
							{
								message = ("Invalid expression for " + tempRuleEffect.getParticipant().getName() + " "
									+ tempTypeRuleEffect.getSimSEObjectType().getName() + " "
										+ SimSEObjectTypeTypes.getText(tempTypeRuleEffect.getSimSEObjectType().getType())
											+ " " + tempAttEffect.getAttribute().getName()
												+ " attribute: number of opening parentheses > number of closing parentheses");
							}
							...
							if((lastToken.trim().equals("-")) || (lastToken.trim().equals(".")) || (lastTokenType == OPEN_PAREN)
								|| (lastTokenType == OPERATOR)) // invalid ending tokens
							{
								if((message != null) && (message.length() > 0)) // message already has some text
								{
									// just concatenate the rest of it:
									message = message.concat("; incomplete expression / invalid ending token");
								}
							...
							if((message != null) && (message.length() > 0)) // message got instantiated
							{
		...
		// current temporary copy of that participant with those values for the object type in focus
	{
		...
		if(objectTypeInFocus != null)
		{
			...
			for(int i=0; i<attributes.size(); i++)
			{
				...
				ParticipantAttributeRuleEffect attEffect =
					ruleInFocus.getParticipantRuleEffect(participantInFocus.getName()).getParticipantTypeEffect(objectTypeInFocus).getAttributeEffect(tempAtt.getName());
			...
			{
				ruleInFocus.getParticipantRuleEffect(participantInFocus.getName()).getParticipantTypeEffect(objectTypeInFocus).setOtherActionsEffect(OtherActionsEffect.ACTIVATE_ALL);
			}
			else if(deactivateButton.isSelected()) // deactivate all
			{
				ruleInFocus.getParticipantRuleEffect(participantInFocus.getName()).getParticipantTypeEffect(objectTypeInFocus).setOtherActionsEffect(OtherActionsEffect.DEACTIVATE_ALL);
			}
			else if(noneButton.isSelected()) // none
			{
				ruleInFocus.getParticipantRuleEffect(participantInFocus.getName()).getParticipantTypeEffect(objectTypeInFocus).setOtherActionsEffect(OtherActionsEffect.NONE);
			}			
		}
	}


	private void setParticipantInFocus(ActionTypeParticipant part, SimSEObjectType type) // makes the specified participant and object
		// type the focus of the gui
	{
		participantInFocus = part;
		objectTypeInFocus = type;

		partEffectsTitle.setText(participantInFocus.getName() + " " + objectTypeInFocus.getName() + " "
			+ SimSEObjectTypeTypes.getText(objectTypeInFocus.getType()) + " Effects:"); // set the title
		...
		for(int index=0; index<attributes.size(); index++) // go through each one and add its info:
		{
			...
			// make it so whenever the text field gains focus, the button pad will be refreshed:
			FocusListener l = new FocusListener ()
			{
				public void focusGained (FocusEvent ev)
				{
					if(!justClosedButtonPad) // valid focus gained, not just as a result of closing the button pad -- if you don't do this, it
						// will bring the first text field into focus every time you close the button pad.
					{
						if(lastFocusedTextField != null) // there was a text field in focus before
						{
		...
		if(ruleInFocus.getParticipantRuleEffect(participantInFocus.getName()).getParticipantTypeEffect(objectTypeInFocus).getOtherActionsEffect().equals(OtherActionsEffect.ACTIVATE_ALL))
		{
			activateButton.setSelected(true);
			deactivateButton.setSelected(false);
			noneButton.setSelected(false);
		}
		else if(ruleInFocus.getParticipantRuleEffect(participantInFocus.getName()).getParticipantTypeEffect(objectTypeInFocus).getOtherActionsEffect().equals(OtherActionsEffect.DEACTIVATE_ALL))
		{
	...
	private void newRuleInput() // creates a new rule input and adds it to the action in focus
	{
		String response = JOptionPane.showInputDialog(null, "Enter a name for this Rule Input:", "Enter Rule Input Name",
			JOptionPane.QUESTION_MESSAGE); // Show input dialog
		if(response != null)
		{
			if(ruleInputNameInputValid(response) == false) // input is invalid
			{
				JOptionPane.showMessageDialog(null, "Please enter a unique name, between 1 and 40 alphanumeric characters, and no spaces", "Invalid Input",
			...
			else // user has entered valid input
			{
				...
				RuleInputInfoForm rInfoForm = new RuleInputInfoForm(this, ruleInFocus, newInput);

				// Makes it so this gui will refresh itself after this rule input info form closes:
	...
	private boolean ruleInputNameInputValid(String input) // returns true if input is a valid rule input name, false if not
	{
		...
		{
			if((Character.isLetterOrDigit(cArray[i])) == false) // character is not a letter or a digit (hence, invalid)
			{
				return false;
			}
		}
	...
	private void editRuleInput(RuleInput input) // brings up the window for editing an existing rule input
	{
		RuleInputInfoForm rInfoForm = new RuleInputInfoForm(this, ruleInFocus, input);
	...
	private void removeRuleInput(String inputName) // removes the rule input with the specified name from the rule in focus
	{

		int choice = JOptionPane.showConfirmDialog(null, ("Really remove " + inputName + "rule input?"), "Confirm Rule Input Removal",
			JOptionPane.YES_NO_OPTION);
		if(choice == JOptionPane.YES_OPTION)
		{
			ruleInFocus.removeRuleInput((String)ruleInputList.getSelectedValue()); // remove rule input from rule in focus
			viewEditInputButton.setEnabled(false);
			removeInputButton.setEnabled(false);
			refreshRuleInputList();
		}
	...
	private void newButtonPad() // creates a new button pad for the participant in focus
	{
		...
		buttGUI = new ButtonPadGUI(this, inputButton, attributeThisPartButton, attributeOtherPartButton, numObjectsButton,
			numActionsThisPartButton, numActionsOtherPartButton, timeButtons, digitButtons, operatorButtons, otherButtons, echoedTextField);
	...
	private void refreshButtonPad() // refreshes the button pad by enabling/disabling buttons
		// according to the attribute in focus and what's in the field in focus
	{
			...
			// attribute
		{
			...
			{
				// enable/disable input button, depending on what kind of inputs currently exist:
				for(int i=0; i<inputs.size(); i++)
				{
					if(((RuleInput)inputs.elementAt(i)).getType().equals(InputType.STRING)) // string rule input type
					{
			...
			else if(attInFocus.getType() == AttributeTypes.BOOLEAN) // boolean attribute
			{
				// enable/disable input button, depending on what kind of inputs currently exist:
				for(int i=0; i<inputs.size(); i++)
				{
					if(((RuleInput)inputs.elementAt(i)).getType().equals(InputType.BOOLEAN)) // boolean rule input type
					{
						inputButton.setEnabled(true);
						break;
					}
			...
			// attribute
		{
			...
			for(int i=0; i<inputs.size(); i++)
			{
				if((((RuleInput)inputs.elementAt(i)).getType().equals(InputType.INTEGER)) ||
					(((RuleInput)inputs.elementAt(i)).getType().equals(InputType.DOUBLE))) // numerical rule input type
				{
					inputButton.setEnabled(true);
					break;
				}
			...
			refreshButtonPad(OPERATOR); // refresh the button pad as if an operator has just been entered
		}
	}


	private void refreshButtonPad(int lastTokenType) // refreshes the button pad by disabling buttons according to what the last entry into
		// the field was
	{
		if((getAttributeInFocus().getType() == AttributeTypes.STRING) || (getAttributeInFocus().getType() == AttributeTypes.BOOLEAN)) 
			// boolean or string attribute
		{
			if((lastFocusedTextField.getText() != null) && (lastFocusedTextField.getText().length() > 0)) // non-empty text field
			{
				disableAllButtons();
			}
			...
			{
				// Rule input button:
				if(inputButton.isEnabled()) // if button is enabled, check if you need to disable it or not:
				{
					Attribute attInFocus = getAttributeInFocus();
					Vector inputs = ruleInFocus.getAllRuleInputs();
					if(attInFocus.getType() == AttributeTypes.STRING) // string attribute
		...
		else
		{
			...
			else if(lastTokenType == DIGIT)
			{
				...
				if(getLastToken(textFieldText).indexOf('.') != -1) // dot has already appeared in this number
				{
				...
				if(getLastToken(lastFocusedTextField.getText()).equals("-")) // negative sign
				{
			...
			if(inputButton.isEnabled()) // if button is enabled, check if you need to disable it or not:
			{
				Attribute attInFocus = getAttributeInFocus();
				Vector inputs = ruleInFocus.getAllRuleInputs();
				if((attInFocus.getType() == AttributeTypes.INTEGER) || (attInFocus.getType() == AttributeTypes.DOUBLE)) // integer or double
					// attribute
				{
					// enable or disable input button, depending on the types of inputs that currently exist:
					boolean disableButt = true;
	...
	private int numOpenParen() // returns the number of open parentheses that are in the last focused text field
	{
		int num = 0;
	...
	private int numClosingParen() // returns the number of closing parentheses that are in the last focused text field
	{
		int num = 0;
		char[] text = lastFocusedTextField.getText().toCharArray();
		for(int i=0; i<text.length; i++)
	...
	private void disableAllButtons() // disables all buttons in the button pad GUI (except backspace)
	{
		inputButton.setEnabled(false);
		attributeThisPartButton.setEnabled(false);
		attributeOtherPartButton.setEnabled(false);
	...
	private Attribute getAttributeInFocus() // returns the Attribute that is currently in focus, according to the most recently focused
		// text field
	{
	...
	private void inputButtonChosen() // takes care of what happens when the user clicks the input button on the button pad
	{
		Vector inputs = ruleInFocus.getAllRuleInputs();
		...
		else if((attInFocus.getType() == AttributeTypes.INTEGER) || (attInFocus.getType() == AttributeTypes.DOUBLE)) // numerical
			// attribute
		{
			for(int i=0; i<inputs.size(); i++)
			{
				RuleInput tempInput = (RuleInput)inputs.elementAt(i);
				if((tempInput.getType().equals(InputType.INTEGER)) || (tempInput.getType().equals(InputType.DOUBLE)))
				{
					choices.add(((RuleInput)inputs.elementAt(i)).getName());
				}
			}
		...
		String response = (String)JOptionPane.showInputDialog(this, "Choose a rule input:", "Rule Input", JOptionPane.PLAIN_MESSAGE,
			null, choices.toArray(), choiceInFocus);
		if((response != null) && (response.length() > 0)) // a selection was made
		{
			// append the text for the selected input to the text field:
			if((getLastTokenType(lastFocusedTextField.getText()) == DIGIT) && (getLastToken(lastFocusedTextField.getText()).equals("-"))) 
				// negative sign was last token
			{
				setFocusedTextFieldText(lastFocusedTextField.getText().trim().concat("input-" + response + " "));
			}
			else
			{
				setFocusedTextFieldText(lastFocusedTextField.getText().concat("input-" + response + " "));
			}
			if((attInFocus.getType() == AttributeTypes.INTEGER) || (attInFocus.getType() == AttributeTypes.DOUBLE))
			{
				refreshButtonPad(NUMBER);
			}
	...
	private void attributeThisPartButtonChosen() // takes care of what to do when this button is clicked
	{
			...
			// attribute
		{
			...
			{
				Attribute tempAtt = (Attribute)atts.elementAt(i);
				if((tempAtt.getType() == AttributeTypes.INTEGER) || (tempAtt.getType() == AttributeTypes.DOUBLE))
				{
					choices.add(((Attribute)atts.elementAt(i)).getName());
				}
			}
		...
		// bring up dialog for choosing a rule input:
		String response = (String)JOptionPane.showInputDialog(this, "Choose an attribute:", "Attribute", JOptionPane.PLAIN_MESSAGE,
			null, choices.toArray(), choiceInFocus);
		if((response != null) && (response.length() > 0)) // a selection was made
		{
			// append the text for the selected input to the text field:
			if((getLastTokenType(lastFocusedTextField.getText()) == DIGIT) && (getLastToken(lastFocusedTextField.getText()).equals("-"))) 
				// negative sign was last token
			{
				setFocusedTextFieldText(lastFocusedTextField.getText().trim().concat("this" + objectTypeInFocus.getName() + "-"
				+ SimSEObjectTypeTypes.getText(objectTypeInFocus.getType()) + ":" + response + " "));
			}
			else
			{
				setFocusedTextFieldText(lastFocusedTextField.getText().concat("this" + objectTypeInFocus.getName() + "-"
					+ SimSEObjectTypeTypes.getText(objectTypeInFocus.getType()) + ":" + response + " "));
			}
			if((attInFocus.getType() == AttributeTypes.INTEGER) || (attInFocus.getType() == AttributeTypes.DOUBLE))
			{
	...
	private void attributeOtherPartButtonChosen() // takes care of what to do when this button is clicked
	{
		Attribute attInFocus = getAttributeInFocus();
		String oldText = lastFocusedTextField.getText(); // temporarily hold the text from the text field
		...
		OtherParticipantAttributeDialog opad = new OtherParticipantAttributeDialog(this, actionInFocus.getAllParticipants(),
			lastFocusedTextField, echoedTextField, attInFocus.getType(), trimWhitespace);
		if((attInFocus.getType() == AttributeTypes.INTEGER) || (attInFocus.getType() == AttributeTypes.DOUBLE))
		{
			if(lastFocusedTextField.getText().equals(oldText) == false) // text has changed
			{
				refreshButtonPad(NUMBER);
			}
		}
	...
	private void numObjectsButtonChosen() // takes care of what to do when this button is clicked
	{
		String oldText = lastFocusedTextField.getText(); // temporarily hold the text from the text field
		boolean trimWhitespace = false;
		if((getLastTokenType(lastFocusedTextField.getText()) == DIGIT) && (getLastToken(lastFocusedTextField.getText()).equals("-"))) 
		...
		NumParticipantsDialog npd = new NumParticipantsDialog(this, actionInFocus.getAllParticipants(), lastFocusedTextField, 
	...
	private void numActionsThisPartButtonChosen() // takes care of what to do when this button is clicked
	{
		String oldText = lastFocusedTextField.getText(); // temporarily hold the text from the text field
		boolean trimWhitespace = false;
		if((getLastTokenType(lastFocusedTextField.getText()) == DIGIT) && (getLastToken(lastFocusedTextField.getText()).equals("-"))) 
		...
		NumActionsThisPartDialog dialog = new NumActionsThisPartDialog(this, participantInFocus, objectTypeInFocus, actions,
	...
	private void numActionsOtherPartButtonChosen() // takes care of what to do when this button is clicked
	{
		String oldText = lastFocusedTextField.getText(); // temporarily hold the text from the text field
		boolean trimWhitespace = false;
		if((getLastTokenType(lastFocusedTextField.getText()) == DIGIT) && (getLastToken(lastFocusedTextField.getText()).equals("-"))) 
		...
		NumActionsOtherPartDialog dialog = new NumActionsOtherPartDialog(this, actionInFocus.getAllParticipants(), actions,
			lastFocusedTextField, echoedTextField, trimWhitespace);
		if(lastFocusedTextField.getText().equals(oldText) == false) // text has changed
		{
			refreshButtonPad(NUMBER);
	...
	private void actTimeElapsedButtonChosen() // takes care of what to do when this button is clicked
	{
		...
		if((getLastTokenType(lastFocusedTextField.getText()) == DIGIT) && (getLastToken(lastFocusedTextField.getText()).equals("-"))) 
			// negative sign was last token
		{
			setFocusedTextFieldText(lastFocusedTextField.getText().trim().concat(text + " "));
		}
		else
		{
	...
	private void randomButtonChosen() // takes care of what to do when this button is clicked
	{
		String oldText = lastFocusedTextField.getText(); // temporarily hold the text from the text field
		boolean trimWhitespace = false;
		if((getLastTokenType(lastFocusedTextField.getText()) == DIGIT) && (getLastToken(lastFocusedTextField.getText()).equals("-"))) 
		...
		RandomFunctionDialog dialog = new RandomFunctionDialog(this, lastFocusedTextField, echoedTextField, trimWhitespace);
		if(lastFocusedTextField.getText().equals(oldText) == false) // text has changed
		{
			refreshButtonPad(NUMBER);
		}
	...
	private void digitButtonChosen(JButton digitButt) // takes care of what to do when this button is clicked
	{
		//System.out.println("*********** At the beginning of digitButtonChosen for butt w/ this text: *" + digitButt.getText() + "*");
		if(getLastToken(lastFocusedTextField.getText()) == null) // empty text field
		{
			//System.out.println("Last token = null");
			//System.out.println("About to append the text *" + digitButt.getText() + "* plus a space");
			// append the text:
			setFocusedTextFieldText(lastFocusedTextField.getText().concat(digitButt.getText() + " "));
		}
		else // non-empty text field
		{
			//System.out.println("Last token = *" + getLastToken(lastFocusedTextField.getText()) + "*");
			...
			{
				//System.out.println("Last token type is open paren or operator -- about to append *" + digitButt.getText() + "* plus a space");
				// append the text:
				setFocusedTextFieldText(lastFocusedTextField.getText().concat(digitButt.getText() + " "));
			}
			else if(tokenType == DIGIT)
			{
				//System.out.println("Last token type is digit -- about to remove trailing whitespace and append *" + digitButt.getText()
					//+ "* plus a space");
				// append the text, removing trailing white space from what was previously in the text field:
				setFocusedTextFieldText(lastFocusedTextField.getText().trim().concat(digitButt.getText() + " "));
			}
		}
		//System.out.println("About to refresh button pad for digit just chosen");
		refreshButtonPad(DIGIT);
		//System.out.println("************* End of digitButtonChosen for butt w/ this text: *" + digitButt.getText() + "*");
	}


	private void operatorButtonChosen(JButton opButt) // takes care of what to do when this button is clicked
	{
		if(opButt.getText().equals("-"))
		{
			//System.out.println("************* At the beginning of operatorButtonChosen function, and - button chosen");
			String lastToken = getLastToken(lastFocusedTextField.getText());
			//System.out.println("Last token = " + lastToken);
			//System.out.println("Last token type = " + getLastTokenType(lastFocusedTextField.getText()));
			if((lastToken == null) || (lastToken.length() == 0) || (getLastTokenType(lastFocusedTextField.getText()) == OPERATOR) ||
				(getLastTokenType(lastFocusedTextField.getText()) == OPEN_PAREN)) // "-" counts for a negative sign in this case
			{
				//System.out.println("About to call digitButtonChosen -- minus button counts for a digit");
				digitButtonChosen(opButt);
			}
			else // "-" counts for a minus operator
			{
				//System.out.println("About to append minus sign plus a space -- minus button counts for a operator");
				// append text:
				setFocusedTextFieldText(lastFocusedTextField.getText().concat(opButt.getText() + " "));
				//System.out.println("About to refresh button pad for operator just chosen");
				refreshButtonPad(OPERATOR);
			}
			//System.out.println("************* At the end of operatorButtonChosen function, and - button chosen");
		}
		else // another operator
		{
			//System.out.println("A non minus sign operator chosen, about to append text: *" + opButt.getText() + "* plus a space");
			// append text:
			setFocusedTextFieldText(lastFocusedTextField.getText().concat(opButt.getText() + " "));
			//System.out.println("About to refresh button pad for operator just chosen.");
			refreshButtonPad(OPERATOR);
			//System.out.println("************* At the end of operatorButtonChosen function, for button *" + opButt.getText() + "*");
		}
	}


	private void stringButtonChosen() // takes care of what to do when this button is clicked
	{
		String response = JOptionPane.showInputDialog(null, "Enter the string:", "String",
			JOptionPane.QUESTION_MESSAGE); // Show input dialog
		if(response != null)
		{
			...
			// Check for length constraints:
			if((cArray.length == 0) || (cArray.length > 500)) // user has entered nothing or a string longer than 500 chars
			{
				JOptionPane.showMessageDialog(null, "Please enter a string between 1 and 500 characters", "Invalid Input",
											 	JOptionPane.WARNING_MESSAGE); // warn user to enter valid input
	...
	private void booleanButtonChosen() // takes care of what to do when this button is clicked
	{
		Object[] choices = {"true", "false"};
		// bring up dialog for choosing a boolean value:
		String response = (String)JOptionPane.showInputDialog(this, "Choose a Boolean value:", "Boolean", JOptionPane.PLAIN_MESSAGE,
	...
	private void backspaceButtonChosen() // takes care of what to do when this button is clicked
	{
		if((getAttributeInFocus().getType() == AttributeTypes.STRING) || (getAttributeInFocus().getType() == AttributeTypes.BOOLEAN))
		...
		{
			String lastToken = getLastToken(lastFocusedTextField.getText());
			if((lastToken != null) && (lastToken.length() > 0)) // there was text in the field
			{
				if(getLastTokenType(lastFocusedTextField.getText()) == DIGIT) // digit token
				{
					// remove the last character:
					setFocusedTextFieldText(lastFocusedTextField.getText().trim().substring(0,
				...
				else // non-digit token
				{
					...
					setFocusedTextFieldText(lastFocusedTextField.getText().trim().substring(0,
						(lastFocusedTextField.getText().trim().length() - lastToken.length())));
				}
				if((getLastTokenType(lastFocusedTextField.getText()) != -1) &&
					(getLastTokenType(lastFocusedTextField.getText()) != OPEN_PAREN)) // last token was not null, not digit, and not open paren
				{
					// add a space to the text field:
					setFocusedTextFieldText(lastFocusedTextField.getText().trim() + " ");
				}				
				if((lastFocusedTextField.getText() == null) || (lastFocusedTextField.getText().length() == 0)) // all of the text has been
					// removed
				{
	...
	private int getLastTokenType(String tokenString) // returns the type of the last token in the token string
	{
		//System.out.println("************* At the beginning of getLastTokenType with *" + tokenString + "* as thte argument");
		...
		if((token == null) || (token.length() == 0)) // no last token
		{
			...
			//System.out.println("*********At the end of getLastTokenType for tokenstring = *" + tokenString + "*, and about to return -1");
			return -1;
		}
		if((token.startsWith("input-")) || (token.startsWith("this")) || (token.startsWith("all")) || (token.startsWith("num"))
			|| (token.startsWith("total")) || (token.startsWith("action")) || (token.startsWith("random"))
				|| (token.startsWith("-input-")) || (token.startsWith("-this")) || (token.startsWith("-all"))
					|| (token.startsWith("-num")) || (token.startsWith("-total")) || (token.startsWith("-action"))
						|| (token.startsWith("-random")))
		{
			//System.out.println("*********At the end of getLastTokenType for tokenstring = *" + tokenString + "*, and about to return NUMBER");
			return NUMBER;
		}
		else if(token.equals("+") || token.equals("*") || token.equals("/"))
		{
			//System.out.println("*********At the end of getLastTokenType for tokenstring = *" + tokenString + "*, and about to return OPERATOR");
			return OPERATOR;
		}
		else if(token.equals("-"))
		{
			...
			String lastLastToken = tokenString.substring(0, (tokenString.trim().length() - token.length())).trim();
			...
			if((lastLastTokenType == OPERATOR) || (lastLastTokenType == OPEN_PAREN) || (lastLastTokenType == -1)) // "-" counts for a
				// negative sign in this case
			{
			//System.out.println("*********At the end of getLastTokenType for tokenstring = *" + tokenString + "*, and about to return DIGIT");
				return DIGIT;
			}
			else //((lastLastTokenType == NUMBER) || (lastLastTokenType == DIGIT)) // "-" counts for a minus sign in this case
			{
			//System.out.println("*********At the end of getLastTokenType for tokenstring = *" + tokenString + "*, and about to return OPERATOR");
				return OPERATOR;
			}
		...
		{
			//System.out.println("*********At the end of getLastTokenType for tokenstring = *" + tokenString + "*, and about to return OPEN_PAREN");
			return OPEN_PAREN;
		}
		else if(token.equals(")"))
		{
			//System.out.println("*********At the end of getLastTokenType for tokenstring = *" + tokenString + "*, and about to return CLOSED_PAREN");
			return CLOSED_PAREN;
		}
		else
		{
	...
	private String getLastToken(String tokenString) // returns the last token in the token string
	{
			...
			// spaces & no parentheses -- must be only one token
		{
			...
			if((tokenString.startsWith("-")) && (tokenString.trim().equals("-") == false)) // begins w/ a neg. sign but is not just a
				// neg. sign
			{
		...
		else // multiple tokens
		{
			...
			String lastBlock = tokenString.trim().substring(tokenString.trim().lastIndexOf(' ') + 1);
			...
			//System.out.println("********** At the end of getLastToken for tokenString = *" + tokenString +
				...
				return lastBlock.substring(1); // return what comes after the parenthesis
			}
			else if(lastBlock.indexOf("(") > 0) // open paren somewhere in the middle
			{
				return lastBlock.substring(lastBlock.lastIndexOf("(") + 1);
			}
			else if((lastBlock.startsWith("-")) && (lastBlock.trim().equals("-") == false)) // begins w/ a neg. sign but is not just a
				// neg. sign
			{
	...
	private void setFocusedTextFieldText(String text) // sets the last focused text field, as well as the echo of this text field in the
