/* This class defines the window through which continuous rules can be put in order for priority of execution*/
...
public class ContinuousRulePrioritizer extends JDialog implements ActionListener
{
	...
	public ContinuousRulePrioritizer(JFrame owner, DefinedActionTypes acts)
	{
		super(owner, true);
		actions = acts;
		...
		for(int i=0; i<allActions.size(); i++)
		{
			...
			Vector rules = tempAct.getAllContinuousRules();
			...
			for(int j=0; j<rules.size(); j++)
			{
				...
				else // priority >= 0
				{
					...
					else
					{
						...
						{
							String tempRuleInfo = (String)prioritizedRules.elementAt(k);
							ActionType tempActType = actions.getActionType(tempRuleInfo.substring((tempRuleInfo.indexOf('(') + 1),
								(tempRuleInfo.length() - 1)));
							Rule tempR = tempActType.getRule(tempRuleInfo.substring(0, (tempRuleInfo.indexOf('(') - 1)));
							if(priority <= tempR.getPriority())
							{
								prioritizedRules.insertElementAt(ruleInfo, k); // insert the rule info
								break;
							}
							else if(k == (prioritizedRules.size() - 1)) // on the last element
							{
								prioritizedRules.add(ruleInfo); // add the rule info to the end of the list
								break;
							}
						}
					}
	...
	public void actionPerformed(ActionEvent evt) // handles user actions
	{
		...
		else if(source == okButton) // okButton has been pressed
		{
			...
			{
				String tempRuleInfo = (String)nonPrioritizedRules.elementAt(i);
				ActionType tempActType = actions.getActionType(tempRuleInfo.substring((tempRuleInfo.indexOf('(') + 1),
					(tempRuleInfo.length() - 1)));
				Rule tempR = tempActType.getRule(tempRuleInfo.substring(0, (tempRuleInfo.indexOf('(') - 1)));
				tempR.setPriority(-1);
			...
			{
				String tempRuleInfo = (String)prioritizedRules.elementAt(i);
				ActionType tempActType = actions.getActionType(tempRuleInfo.substring((tempRuleInfo.indexOf('(') + 1),
					(tempRuleInfo.length() - 1)));
				Rule tempR = tempActType.getRule(tempRuleInfo.substring(0, (tempRuleInfo.indexOf('(') - 1)));
				tempR.setPriority(i);
