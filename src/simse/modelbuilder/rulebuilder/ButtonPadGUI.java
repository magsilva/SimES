/* This class is the GUI responsible for displaying the button pad for specifying effects on attributes for EffectRules with */
...
{	
	...
	private JTextArea echoedTextField; // text field from the effect rule info form to be echoed under the button pad
	private JButton okButton;

	public ButtonPadGUI(JDialog owner, JButton input, JButton attsThis, JButton attsOther, JButton numObjects, JButton numActionsThis,
		JButton numActionsOther, Vector time, Vector digits, Vector operators, Vector other, JTextArea echoedTField)
	{
		super(owner, true);
		...
		echoedPane.setHorizontalScrollBarPolicy(JScrollPane.HORIZONTAL_SCROLLBAR_AS_NEEDED);
		...
		repaint();
		toFront();
		// Make it show up in the center, lower half of the screen, slightly above center:
		Point ownerLoc = owner.getLocationOnScreen();
		Point thisLoc = new Point();
		thisLoc.setLocation((ownerLoc.getX() + (owner.getWidth() / 2) - (this.getWidth() / 2)),
			(ownerLoc.getY() + (owner.getHeight() / 2) - 200));
		setLocation(thisLoc);
		setVisible(true);
	}
