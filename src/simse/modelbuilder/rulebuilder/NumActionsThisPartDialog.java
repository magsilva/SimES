/* This class defines the dialog for choosing a number of actions for the participant in focus for the effect rule info form */
...
public class NumActionsThisPartDialog extends JDialog implements ActionListener
{
	...
	private boolean trimWhitespace; // whether or not to trim trailing whitespace in the text field b4 appending to it
	...
	public NumActionsThisPartDialog(JDialog owner, ActionTypeParticipant participantInFocus, SimSEObjectType objectTypeInFocus,
		DefinedActionTypes acts, JTextField tField, JTextArea echoedTField, boolean trim)
	{
		super(owner, true);
	...
	public void actionPerformed(ActionEvent evt) // handles user actions
	{
		...
		else if(source == okButton)
		{
			...
			String typeString = (objType.getName() + "-" + SimSEObjectTypeTypes.getText(objType.getType()));
			...
			if(trimWhitespace)
			{
				setFocusedTextFieldText(fieldInFocus.getText().trim().concat(activeString + "-" + partString + "-" + typeString 
					+ ":" + actionString + " "));
			}
			else
			{
				setFocusedTextFieldText(fieldInFocus.getText().concat(activeString + "-" + partString + "-" + typeString + ":" + actionString
					+ " "));
			}
			setVisible(false);
			dispose();
