...
public class RuleBuilderGUI extends JPanel implements ActionListener, ListSelectionListener, MouseListener
{
	...
	private JButton addCreateObjectsRuleButton; // button for adding a new create objects rule
	private JButton addDestroyObjectsRuleButton; // button for adding a new destroy objects rule
	private JList definedActionsList; // JList of defined actions
	//private JButton viewEditRulesButton; // button for viewing/editing the rules of an action

	public RuleBuilderGUI(ModelBuilderGUI owner, DefinedObjectTypes objTypes, DefinedActionTypes acts)
	{
		mainGUI = owner;
		actions = acts;
		objects = objTypes;
		...
		ruleTablePane.setVerticalScrollBarPolicy(JScrollPane.VERTICAL_SCROLLBAR_ALWAYS);
	...
	{	    
	    ...
	    if (me.getButton() == MouseEvent.BUTTON1 && clicks >= 2)
	    {
			...
			{
				Rule tempRule =
					ruleTblMod.getActionTypeInFocus().getRule((String)(ruleTblMod.getValueAt(ruleTable.getSelectedRow(), 0)));
				editRule(tempRule);
				editRuleButton.setEnabled(false);
				removeRuleButton.setEnabled(false);
				renameRuleButton.setEnabled(false);
	...
	public void actionPerformed(ActionEvent evt) // handles user actions
	{
		...
		else if(source == removeRuleButton)
		{
			if(ruleTable.getSelectedRow() >= 0) // a row is selected
			{
				Rule tempRule =	ruleTblMod.getActionTypeInFocus().getRule((String)(ruleTblMod.getValueAt(ruleTable.getSelectedRow(), 0)));
				removeRule(tempRule);
			}
		}
	...
	private void newCreateObjectsRule() // creates a new create objects rule and adds it to the action in focus
	{
		String response = JOptionPane.showInputDialog(null, "Enter a name for this Create Objects Rule:", "Enter Rule Name",
			JOptionPane.QUESTION_MESSAGE); // Show input dialog
		if(response != null)
		{
			if(ruleNameInputValid(response) == false) // input is invalid
			{
				JOptionPane.showMessageDialog(null, "Please enter a unique name, between 2 and 40 alphabetic characters, and no spaces",
					"Invalid Input", JOptionPane.WARNING_MESSAGE); // warn user to enter a valid name
				newCreateObjectsRule(); // try again
			}
			else // user has entered valid input
			{
				CreateObjectsRule newRule = new CreateObjectsRule(response, ruleTblMod.getActionTypeInFocus()); // create new rule
				CreateObjectsRuleInfoForm rInfoForm = new CreateObjectsRuleInfoForm(mainGUI, newRule, ruleTblMod.getActionTypeInFocus(),
					actions, objects, true);

				// Makes it so this gui will refresh itself after this rule info form closes:
	...
	private void newDestroyObjectsRule() // creates a new destroy objects rule and adds it to the action in focus
	{
		...
		if(response != null)
		{
			...
			{
				DestroyObjectsRule newRule = new DestroyObjectsRule(response, ruleTblMod.getActionTypeInFocus()); // create new rule
				DestroyObjectsRuleInfoForm dInfoForm = new DestroyObjectsRuleInfoForm(mainGUI, newRule, ruleTblMod.getActionTypeInFocus(),
					actions, true);
	...
	private void newEffectRule() // creates a new effect rule and adds it to the action in focus
	{
		...
		if(response != null)
		{
			...
			{
				EffectRule newRule = new EffectRule(response, ruleTblMod.getActionTypeInFocus()); // create new rule
				// create a ParticipantRuleEffect for each participant in the action type:
				Vector parts = ruleTblMod.getActionTypeInFocus().getAllParticipants();
				for(int i=0; i<parts.size(); i++)
				{
					newRule.addParticipantRuleEffect(new ParticipantRuleEffect((ActionTypeParticipant)parts.elementAt(i)));
				}
				EffectRuleInfoForm rInfoForm = new EffectRuleInfoForm(mainGUI, newRule, ruleTblMod.getActionTypeInFocus(),
					actions, objects, true);
	...
	private void editRule(Rule rule)
	{
		...
		{
			CreateObjectsRuleInfoForm rInfoForm = new CreateObjectsRuleInfoForm(mainGUI, (CreateObjectsRule)rule,
				ruleTblMod.getActionTypeInFocus(), actions, objects, false);
		...
		{
			EffectRuleInfoForm rInfoForm = new EffectRuleInfoForm(mainGUI, (EffectRule)rule, ruleTblMod.getActionTypeInFocus(),
	...
	private void renameRule(Rule rule) // removes the action with the specified name from the data structure
	{
	    
		String response = JOptionPane.showInputDialog(null, "Enter new name for " + rule.getName(),
				"Rename Rule Type", JOptionPane.QUESTION_MESSAGE); // Show input dialog
		if(response != null)
		{
			if(ruleNameInputValid(response) == false) // input is invalid
			{
	...
	{
		int choice = JOptionPane.showConfirmDialog(null, ("Really remove " + rule.getName() + " rule?"),
			"Confirm Rule Removal", JOptionPane.YES_NO_OPTION);
	...
	private boolean ruleNameInputValid(String input) // returns true if input is a valid rule name, false if not
	{
		...
		for(int i=0; i<existingActionTypes.size(); i++)
		{
			...
			for(int j=0; j<existingRules.size(); j++)
			{
				...
				if(tempRule.getName().equalsIgnoreCase(input)) // name entered is not unique (there is already another rule defined with the
	...
	private void setupRuleTableSelectionListenerStuff() // enables edit and remove rule buttons whenever a row (rule) is selected
	{
	...
	private void setupDefinedActionsListSelectionListenerStuff() // enables view/edit rules button whenever a list item (action) is selected
	{
		// Copied from a Java tutorial:
	...
	private void setActionInFocus(ActionType newAct) // sets the given action type as the focus of this GUI
	{
		ruleTblMod.setActionTypeInFocus(newAct); // set focus of rule table to new action type
		ruleTableTitle.setText((newAct.getName()) + " Action Rules:"); // change title of table to reflect new action
	...
	private void clearActionInFocus() // clears the GUI so that it doesn't have an action type in focus
	{
