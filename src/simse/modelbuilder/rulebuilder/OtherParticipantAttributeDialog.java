/* This class defines the dialog for choosing an attribute of another participant for the effect rule info form */
...
public class OtherParticipantAttributeDialog extends JDialog implements ActionListener
{
	...
	boolean trimWhitespace; // whether or not to trim the trailing whitespace b4 appending to the text field
	...
	public OtherParticipantAttributeDialog(JDialog owner, Vector parts, JTextField tField, JTextArea echoedTField, int attType,
		boolean trim)
	{
		super(owner, true);
		...
		// bottom pane (had to put it first because refreshAttList() references the okButton):
	...
	public void actionPerformed(ActionEvent evt) // handles user actions
	{
		...
		else if(source == okButton)
		{
			...
			if(trimWhitespace)
			{
				setFocusedTextFieldText(fieldInFocus.getText().trim().concat(activeString + "-" + partString + ":" + attString + " "));
			}
			else
			{
				setFocusedTextFieldText(fieldInFocus.getText().concat(activeString + "-" + partString + ":" + attString + " "));
			}
			setVisible(false);
			dispose();
		}
	...
	private void refreshAttList()
	{
		...
		String typeName = part.substring((part.indexOf(' ') + 1), part.lastIndexOf(' '));
		...
		for(int i=0; i<participants.size(); i++)
		{
			...
			if(tempPart.getName().equals(partName)) // found it
			{
				...
				for(int j=0; j<atts.size(); j++)
				{
					...
					if((attributeType == AttributeTypes.INTEGER) || (attributeType == AttributeTypes.DOUBLE))
						// numerical attribute
					{
						if((tempAtt.getType() == AttributeTypes.INTEGER) || (tempAtt.getType() == AttributeTypes.DOUBLE)) // numerical
							// attribute also
						{
					...
					{
						if(tempAtt.getType() == AttributeTypes.STRING) // string attribute also
						{
							attList.addItem(tempAtt.getName());
						}
					}
					else if(attributeType == AttributeTypes.BOOLEAN) // boolean attribute
					{
						if(tempAtt.getType() == AttributeTypes.BOOLEAN) // boolean attribute also
						{
							attList.addItem(tempAtt.getName());
						}
					}
		...
		{
			okButton.setEnabled(false); // don't allow the user to press ok with an empty attribute selection
		}
		else
		{
