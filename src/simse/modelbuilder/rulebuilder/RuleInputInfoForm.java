/* This class defines the window through which info about a rule input can be added/edited */
...
public class RuleInputInfoForm extends JDialog implements ActionListener
{
	...
	private boolean newInput; // whether this is a new RuleInput being created (true) or an existing one being edited (false)
	...
	private JCheckBox cancelableBox; // for specifying whether or not it's cancelable
	...
	public RuleInputInfoForm(JDialog owner, EffectRule rule, RuleInput input)
	{
		...
		{
			if(((RuleInput)inputs.elementAt(i)) == inputInFocus) // input in focus already exists in the rule in focus
			newInput = false;
		}
	...
	public void actionPerformed(ActionEvent evt) // handles user actions
	{
		...
		else if(source == typeList)
		{
			...
			if((selectedString.equals(InputType.STRING))
				|| (selectedString.equals(InputType.BOOLEAN))) // string or boolean selected
			{
	...
	private Vector validateInput() // returns a vector of error messages (if any)
	{
		...
		if((prompt == null) || (prompt.length() == 0) || (prompt.length() > 400)) // string empty or longer than 400 chars
		{
		...
		if((condition != null) && (condition.length() > 0)) // not blank
		{
			if(type.equals(InputType.INTEGER)) // integer type
			{
				...
				{
					messages.add("Please enter a valid integer for the condition or else leave it blank");
				}
			}
			else if(type.equals(InputType.DOUBLE)) // double type
			{
				...
				{
					messages.add("Please enter a valid double value for the condition or else leave it blank");
				}
			}
		}
		return messages;
	...
	private void addEditInput() // sets the values of the RuleInput in focus from the existing info in the form and adds it to the
		// rule in focus. Note: validateInput() should be called before calling this function to ensure that you're adding a valid rule
		// input
	{
		String type = (String)typeList.getSelectedItem();
		inputInFocus.setType(type); // set type
		if((type.equals(InputType.STRING)) || (type.equals(InputType.BOOLEAN))) // string or boolean
		{
			// clear condition:
		...
		{
			inputInFocus.getCondition().setGuard((String)guardList.getSelectedItem()); // set guard
			String condition = conditionTextField.getText();
			try
			{
				int intVal = Integer.parseInt(condition);
				inputInFocus.getCondition().setValue(new Integer(intVal)); // set condition value
			}
			catch(NumberFormatException e)
			{
				System.out.println(e.getMessage()); // NOTE: this exception should never be thrown since validateInput() should
					// always be called before this function to ensure that the input is valid
			}
		}
		else if(type.equals(InputType.DOUBLE)) // double input type
		{
			...
			{
				double doubleVal = Double.parseDouble(condition);
				inputInFocus.getCondition().setValue(new Double(doubleVal)); // set condition value
			}
			catch(NumberFormatException e)
			{
				System.out.println(e.getMessage()); // NOTE: this exception should never be thrown since validateInput() should
		...
		{
			ruleInFocus.addRuleInput(inputInFocus); // add the input to the rule in focus
		}
	...
	{
		if(!newInput) // editing existing input; initialize form with input's values
		{
			typeList.setSelectedItem(inputInFocus.getType()); // set type
			guardList.setSelectedItem(inputInFocus.getCondition().getGuard()); // set guard
			if(inputInFocus.getCondition().isConstrained())
			{
				conditionTextField.setText(inputInFocus.getCondition().getValue().toString()); // set condition value
			}
