/* This class defines the dialog for choosing a number of participants in an action for the effect rule info form */
...
public class NumParticipantsDialog extends JDialog implements ActionListener
{
	...
	private boolean trimWhitespace; // whether or not to trip the trailing whitespace in the text field b4 appending to it
	...
	public NumParticipantsDialog(JDialog owner, Vector parts, JTextField tField, JTextArea echoedTField, boolean trim)
	{
		super(owner, true);
	...
	public void actionPerformed(ActionEvent evt) // handles user actions
	{
		...
		else if(source == okButton)
		{
			...
			if(trimWhitespace)
			{
				setFocusedTextFieldText(fieldInFocus.getText().trim().concat(activeString + "-" + partString + " "));
			}
			else
			{
				setFocusedTextFieldText(fieldInFocus.getText().concat(activeString + "-" + partString + " "));
			}
			setVisible(false);
			dispose();
		}
