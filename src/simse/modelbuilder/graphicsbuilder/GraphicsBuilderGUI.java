...
public class GraphicsBuilderGUI extends JPanel implements ActionListener
{
	...
	private CreatedObjects objects; // data structure for holding all of the created SimSE objects
	...
	private Hashtable startStateObjsToImgFilenames; // maps SimSEObjects (keys) from the start state to the filename of the associated image file (String)
	private Hashtable ruleObjsToImgFilenames; // maps SimSEObjects (keys) from the create objects rules to the filename of the associated image file (String)
	private Hashtable imagesToFilenames; // maps ImageIcons (keys) to the filename of that image's file (String) (values)
	...
	private Vector imageButtons; // buttons w/ images on them
	private JLabel topPanelLabel;
	private JLabel bottomPanelLabel;
	private WarningListPane warningPane;

	
	public GraphicsBuilderGUI(ModelBuilderGUI owner, DefinedObjectTypes objTs, CreatedObjects objs, DefinedActionTypes acts)
	{
		mainGUI = owner;
		...
		sopFileManip = new SopFileManipulator(objTypes, objects, actTypes, startStateObjsToImgFilenames, ruleObjsToImgFilenames);	

		// image dir file chooser:
		imageDirChooser = new JFileChooser();
		imageDirChooser.addChoosableFileFilter(new DirectoryFileFilter()); // make it so it only displays directories
		imageDirChooser.setDialogTitle("Please select the directory where available icons are located:");
		...
		iconDirButt.setToolTipText("Change the directory where the icons are located");
		...
		objectList.setSelectionMode(ListSelectionModel.SINGLE_SELECTION); // only allow the user to select one item at a time
	...
	public void reload(File tempFile) // reloads the graphics from a temporary file
	{		
		// reload:
		...
		if(source == matchButton)
		{
			if(objInFocus != null) // there is an object selected
			{
				if(selectedImage.getIcon() != null) // there is an image selected
				{
					...
					String imgFilename = (String)imagesToFilenames.get((ImageIcon)selectedImage.getIcon());
					if(objects.getAllObjects().contains(objInFocus)) // a start state object
					{
				...
				{
					JOptionPane.showMessageDialog(null, "Please choose an image to match", "Match Unsuccessful",
						JOptionPane.WARNING_MESSAGE);
				}
			}
			else
			{
				JOptionPane.showMessageDialog(null, "Please choose an object to match", "Match Unsuccessful",
					JOptionPane.WARNING_MESSAGE);			
			}
		}
	...
	private boolean isImageFile(String filename)
	{
		if(filename.indexOf('.') >= 0)
		{
			...
			if((extension.equalsIgnoreCase("png")) || (extension.equalsIgnoreCase("gif")) || (extension.equalsIgnoreCase("jpg"))
	...
	private void refreshObjectList()
	{
		...
		// append all objects created by CreateObjectsRules to the currentObjs Vector:
		...
		for(int i=0; i<currentObjs.size(); i++)
		{
			...
			data.append(tempObj.getSimSEObjectType().getName() + " " + SimSEObjectTypeTypes.getText(tempObj.getSimSEObjectType().getType()) + " ");
			if(tempObj.getSimSEObjectType().hasKey() && tempObj.getKey().isInstantiated()) // has key and it is instantiated
			{
	...
	private void refreshImagePane()
	{
		...
		{
			String pictureFiles[] = imageDir.list();	// list out all picture filenames and store in pictureFiles[]
			
			for(int i=0; i<pictureFiles.length; i++)
			{
				if(isImageFile(pictureFiles[i])) // to prevent non-image files from being loaded
				{
					ImageIcon img = new ImageIcon(imageDir.getPath().concat("\\" + pictureFiles[i]));
	...
	private void setupObjectListSelectionListenerStuff() // enables match button whenever both an object and an image are selected
	{
		...
		rowSM.addListSelectionListener(new ListSelectionListener(){
			public void valueChanged(ListSelectionEvent lse) {
				...
				{
					// get the selected object information from the selected item in objectList:
					String selectedItem = (String)objectList.getSelectedValue();
					String ssObjType = selectedItem.substring(0, selectedItem.indexOf(' '));
					String temp = selectedItem.substring(selectedItem.indexOf(' ') + 1); // take off SimSEObjectTypeType
					String ssObjTypeType = temp.substring(0, temp.indexOf(' '));
					String keyAttVal = temp.substring(temp.indexOf(' ') + 1);
					int metaType = SimSEObjectTypeTypes.getIntRepresentation(ssObjTypeType);
					
					// set the object in focus:
					SimSEObjectType objType = objTypes.getObjectType(metaType, ssObjType); // get the SimSEObjectType
					if(objType.hasKey()) // objType has a key attribute specified
					{
						...
						{
							// try to get the object from the CreatedObjects:
							objInFocus = objects.getObject(metaType, ssObjType, new Boolean(keyAttVal));
							if(objInFocus == null) // not a start state object
							{
								// get it from the rules:
								objInFocus = getObjectFromRules(metaType, ssObjType, new Boolean(keyAttVal));
							}							
						}
						else // string
						{
					...
					if(startStateObjsToImgFilenames.containsKey(objInFocus)) // object in focus is a start state object and has an image matched to it
					{
						// display the associated image:
						selectedImage.setText(null);
						ImageIcon img = getImage((String)startStateObjsToImgFilenames.get(objInFocus)); // get the image from the filename
						selectedImage.setIcon(img);
					}
					else if(ruleObjsToImgFilenames.containsKey(objInFocus)) // object in focus is a rule-generated object and has an image matched to it
					{
						// display the associated image:
						selectedImage.setText(null);
						ImageIcon img = getImage((String)ruleObjsToImgFilenames.get(objInFocus)); // get the image from the filename
						selectedImage.setIcon(img);
					}
					...
					if((selectedImage.getText() == null) || (selectedImage.getText().length() == 0)) // image selected
					{
						// enable match button:
						matchButton.setEnabled(true);
					}
	...
	private ImageIcon getImage(String filename) // returns the image associated with the specified filename
	{
