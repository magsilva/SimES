...
{
	private String name;
	private Vector participantTriggers; // vector of ActionTypeParticipantTriggers for this action type trigger
	private String triggerText; // text that shows up over the head of an employee participant when this is triggered
	...
	private ActionType action; // POINTER TO action type that this trigger is attached to
	...
	public Object clone()
	{
		try
		{
			...
			{
				clonedTriggers.add((ActionTypeParticipantTrigger)(((ActionTypeParticipantTrigger)(participantTriggers.elementAt(i))).clone()));
			}
			cl.participantTriggers = clonedTriggers;
			cl.triggerText = triggerText;
			cl.priority = priority;
			cl.gameEnding = gameEnding;
			cl.action = action; // NOTE: since this is a pointer to the action, it must remain a pointer to the
	...
	public ActionTypeParticipantTrigger getParticipantTrigger(String participantName) // returns the trigger for the specified participant
	{
		for(int i=0; i<participantTriggers.size(); i++)
		{
			ActionTypeParticipantTrigger tempTrig = (ActionTypeParticipantTrigger)(participantTriggers.elementAt(i));
	...
	public void addParticipantTrigger(ActionTypeParticipantTrigger newTrig) // adds the specified participant trigger to this
		// ActionTypeTrigger; if a trigger for this participant is already there, the new trigger replaces it.
	{
		boolean notFound = true;
		for(int i=0; i<participantTriggers.size(); i++)
		{
			ActionTypeParticipantTrigger tempTrig = (ActionTypeParticipantTrigger)(participantTriggers.elementAt(i));
			if(tempTrig.getParticipant().getName().equals(newTrig.getParticipant().getName())) // trigger for this participant already
	...
	public void addEmptyTrigger(ActionTypeParticipant part) // adds a new participant trigger that is unconstrained to this
		// ActionTypeTrigger; if a trigger for this participant is already there, the new trigger replaces it.
	{
		for(int i=0; i<participantTriggers.size(); i++)
		{
			ActionTypeParticipantTrigger tempTrig = (ActionTypeParticipantTrigger)(participantTriggers.elementAt(i));
			if(tempTrig.getParticipant().getName().equals(part.getName())) // trigger for this participant already exists, needs to be
				// replaced
			{
	...
	public void removeTrigger(String partName) // removes the trigger for the participant with the specified name
	{
		for(int i=0; i<participantTriggers.size(); i++)
		{
			ActionTypeParticipantTrigger tempTrig = (ActionTypeParticipantTrigger)(participantTriggers.elementAt(i));
	...
	public ActionTypeTrigger morph(String triggerType) // morphs this ActionTypeTrigger to the new type of trigger specified and returns
		// it.
	{
		if(triggerType.equals(AUTO)) // autonomous trigger type
		{
			AutonomousActionTypeTrigger autoTrig = new AutonomousActionTypeTrigger(name, action);
		...
		{
			RandomActionTypeTrigger randomTrig = new RandomActionTypeTrigger(name, action);
			if(this instanceof RandomActionTypeTrigger)
			{
				randomTrig.setFrequency(((RandomActionTypeTrigger)(this)).getFrequency());
			}
	...
	public ActionType getActionType() // returns a COPY of the action type that this trigger is attached to
	{
