...
public class ActionTypeDestroyerInfoForm extends JDialog implements
  ...
  public ActionTypeDestroyerInfoForm(JDialog owner, ActionType action,
      ActionTypeDestroyer dest, DefinedActionTypes acts) {
    super(owner, true);
    ...
    menuTextField
        .setToolTipText("Enter the text to be shown on the user's menu for this destroyer");
    ...
    frequencyTextField
        .setToolTipText("Enter the % chance of this destroyer being fired at each clock tick in which all of the destroyer conditions are met");
    ...
    timeTextField
        .setToolTipText("Enter the number of clock ticks that this action will exist for before it is to be automatically destroyed");
    ...
    textTextField
        .setToolTipText("Enter the text to be shown in an employee/customer's overhead bubble when this destroyer is fired");
    ...
    for (int i = 0; i < parts.size(); i++) {
      ...
      if ((tempPart.getSimSEObjectTypeType() == SimSEObjectTypeTypes.EMPLOYEE)
          || (tempPart.getSimSEObjectTypeType() == SimSEObjectTypeTypes.CUSTOMER)) {
  ...
  public void actionPerformed(ActionEvent evt) // handles user actions
  {
    ...
    if (source == destroyerTypeList) {
      if (destroyerTypeList.getSelectedItem().equals(ActionTypeDestroyer.AUTO)) // autonomous
                                                                                // destroyer
                                                                                // type
                                                                                // selected
      {
      ...
      if (errors.size() == 0) // input valid
      {
        ...
        int index = actionInFocus.removeDestroyer(originalDestroyer.getName()); // remove
                                                                                // old
                                                                                // destroyer
  ...
  private void initializeForm() {
                                                              ...
                                                              // timed type
    {
      ...
      timeTextField.setText((new Integer(
          ((TimedActionTypeDestroyer) (destroyer)).getTime())).toString()); // set
                                                                            // time
                                                                            // text
                                                                            // field
  ...
  private Vector inputValid() {
    ...
    {
      messages
          .add("Please enter text for the destroyer's name -- between 2 and 40 alphabetic characters"); // warn
                                                                                                        // user
                                                                                                        // to
                                                                                                        // enter
                                                                                                        // a
    ...
    for (int i = 0; i < cArrayA.length; i++) {
      ...
      {
        messages
            .add("Please enter only alphabetic characters for the destroyer's name"); // warn
                                                                                      // user
                                                                                      // to
                                                                                      // enter
                                                                                      // a
    ...
    for (int i = 0; i < existingDests.size(); i++) {
      ActionTypeDestroyer tempDest = ((ActionTypeDestroyer) existingDests
          .elementAt(i));
      if ((tempDest.getName().equalsIgnoreCase(nameField.getText()))
          && (tempDest != originalDestroyer)) // name entered is not unique
      // (there is already another destroyer defined with the same name (hence,
      // invalid)
      {
    ...
    if ((destroyerText != null) && (destroyerText.length() > 0)) // destroyer
                                                                 // text entered
    {
      if (destroyerText.equalsIgnoreCase("Everyone stop what you're doing")) // reserved
                                                                             // phrase
      {
        messages
    ...
    if (gameEndCBox.isSelected()) // game-ending destroyer
    {
      ...
      {
        messages
            .add("Since this is a game-ending destroyer, you must assign exactly one attribute to be the scoring attribute");
      }
    }
          ...
          for (int j = 0; j < tempDests.size(); j++) {
            ...
            // one in focus.
            {
              ...
              // a destroyer with this menu text
              {
                messages
                    .add(new String(
                        "A user-initiated destroyer with that menu text already exists"));
              }
            }
          }
          ...
          for (int j = 0; j < tempTrigs.size(); j++) {
                                                           ...
                                                           // triggers
            {
              ...
              // trigger with this menu text
              {
                messages
                    .add(new String(
                        "A user-initiated trigger with that menu text already exists"));
              }
            }
          }
        }
