...
public class DestroyerPrioritizer extends JDialog implements ActionListener {
  ...
  private Vector nonPrioritizedDestroyers; // vector of destroyers that haven't
                                           // been prioritized yet
  private Vector prioritizedDestroyers; // vector of destroyers that have been
                                        // prioritized
                                                 ...
                                                 // destroyer list
          {
            ...
            for (int k = 0; k < prioritizedDestroyers.size(); k++) {
              ...
              if (priority <= tempDest.getPriority()) {
                prioritizedDestroyers.insertElementAt(destroyerInfo, k); // insert
                                                                         // the
                                                                         // destroyer
                break;
                                                                  ...
                                                                  // element
              {
                prioritizedDestroyers.add(destroyerInfo); // add the destroyer
                                                          // to the end of the
                                                          // list
                break;
              }
  ...
  public void actionPerformed(ActionEvent evt) // handles user actions
  {
    ...
    if (source == cancelButton) // cancel button has been pressed
    {
      ...
      nonPrioritizedDestroyers.remove(selIndex); // remove destroyer from
                                                 // lefthand list
      ...
      if ((nonPrioritizedDestroyers.size() == 0)
          || (nonPrioritizedDestroyerList.getSelectedIndex() > (nonPrioritizedDestroyers
              .size() - 1))) // no
      // item is selected (had to use nonPrioritizedDestroyers.size() - 1
      // because it doesn't work using selected index = -1 or
      // selected value == null!
      {
        // disable button:
        rightArrowButton.setEnabled(false);
      ...
      String tempStr = (String) (prioritizedDestroyers.elementAt(selIndex)); // remove
                                                                             // destroyer
                                                                             // from
                                                                             // righthand
                                                                             // list
      prioritizedDestroyers.remove(selIndex); // remove destroyer from righthand
                                              // list
      ...
      if ((prioritizedDestroyers.size() == 0)
          || (prioritizedDestroyerList.getSelectedIndex() > (prioritizedDestroyers
              .size() - 1))) // no
      // item is selected (had to use prioritizedDestroyers.size() - 1 because
      ...
      {
        String tempAct = (String) (prioritizedDestroyers.remove(selIndex)); // remove
                                                                            // destroyer
        prioritizedDestroyers.insertElementAt(tempAct, (selIndex - 1)); // move
                                                                        // up
