...
{
	private String name;
	private Vector participantDestroyers; // vector of ActionTypeParticipantDestroyers for this action type destroyer
	private String destroyerText; // text that shows up over the head of an employee participant when this is destroyed
	...
	private ActionType action; // POINTER TO the action that this destroyer is attached to
	...
	public Object clone()
	{
		try
		{
			...
			{
				clonedDestroyers.add((ActionTypeParticipantDestroyer)(((ActionTypeParticipantDestroyer)(participantDestroyers.elementAt(i))).clone()));
			}
			cl.participantDestroyers = clonedDestroyers;
			cl.destroyerText = destroyerText;
			cl.priority = priority;
			cl.gameEnding = gameEnding;		
			cl.action = action; // NOTE: since this is a pointer to the action, it must remain a pointer to the action, even in the clone, 
				// so BE CAREFUL!
			return cl;
		}
	...
	public ActionTypeParticipantDestroyer getParticipantDestroyer(String participantName) // returns the destroyer for the specified
		// participant
	{
		for(int i=0; i<participantDestroyers.size(); i++)
		{
			ActionTypeParticipantDestroyer tempDest = (ActionTypeParticipantDestroyer)(participantDestroyers.elementAt(i));
	...
	public void addParticipantDestroyer(ActionTypeParticipantDestroyer newDest) // adds the specified participant destroyer to this
		// ActionTypeDestroyer; if a destroyer for this participant is already there, the new destroyer replaces it.
	{
		boolean notFound = true;
		for(int i=0; i<participantDestroyers.size(); i++)
		{
			ActionTypeParticipantDestroyer tempDest = (ActionTypeParticipantDestroyer)(participantDestroyers.elementAt(i));
			if(tempDest.getParticipant().getName().equals(newDest.getParticipant().getName())) // destroyer for this participant already
	...
	public void addEmptyDestroyer(ActionTypeParticipant part) // adds a new participant destroyer that is unconstrained to this
		// ActionTypeDestroyer; if a destroyer for this participant is already there, the new destroyer replaces it.
	{
		for(int i=0; i<participantDestroyers.size(); i++)
		{
			ActionTypeParticipantDestroyer tempDest = (ActionTypeParticipantDestroyer)(participantDestroyers.elementAt(i));
			if(tempDest.getParticipant().getName().equals(part.getName())) // destroyer for this participant already exists, needs to be
				// replaced
			{
	...
	public void removeDestroyer(String partName) // removes the destroyer for the participant with the specified name
	{
		for(int i=0; i<participantDestroyers.size(); i++)
		{
			ActionTypeParticipantDestroyer tempDest = (ActionTypeParticipantDestroyer)(participantDestroyers.elementAt(i));
	...
	public ActionTypeDestroyer morph(String destroyerType) // morphs this ActionTypeDestroyer to the new type of destroyer specified and
		// returns it.
	{
		if(destroyerType.equals(AUTO)) // autonomous destroyer type
		{
			AutonomousActionTypeDestroyer autoDest = new AutonomousActionTypeDestroyer(name, action);
		...
		{
			UserActionTypeDestroyer userDest = new UserActionTypeDestroyer(name, action);
		...
		{
			RandomActionTypeDestroyer randomDest = new RandomActionTypeDestroyer(name, action);
			if(this instanceof RandomActionTypeDestroyer)
			{
				randomDest.setFrequency(((RandomActionTypeDestroyer)(this)).getFrequency());
			}
		...
		{
			TimedActionTypeDestroyer timedDest = new TimedActionTypeDestroyer(name, action);
	...
	public ActionType getActionType() // returns a COPY of the action type that this destroyer is attached to
	{
