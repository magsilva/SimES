...
public class ActionTypeTriggerInfoForm extends JDialog implements ActionListener
{
	...
	private JComboBox triggerTypeList; // for choosing type of trigger (autonomous, user, or random)
	...
	private JList participantList; // for choosing a participant whose trigger conditions to edit
	private JButton viewEditButton; // for viewing/editing trigger conditions
	private JCheckBox gameEndCBox; // for indicating that this trigger is a game-ending trigger
	...
	public ActionTypeTriggerInfoForm(JDialog owner, ActionType action, ActionTypeTrigger trig, DefinedActionTypes acts)
	{
		super(owner, true);
		originalTrigger = trig; // store pointer to original
		if(originalTrigger instanceof AutonomousActionTypeTrigger)
		{
			trigger = (AutonomousActionTypeTrigger)(trig.clone()); // make a copy of the trigger for temporary editing
		}
		else if(originalTrigger instanceof UserActionTypeTrigger)
		{
			trigger = (UserActionTypeTrigger)(trig.clone()); // make a copy of the trigger for temporary editing
		}
		else if(originalTrigger instanceof RandomActionTypeTrigger)
		{
			trigger = (RandomActionTypeTrigger)(trig.clone()); // make a copy of the trigger for temporary editing
		}
		...
		menuTextField.setToolTipText("Enter the text to be shown on the user's menu for this trigger");
		...
		frequencyTextField.setToolTipText("Enter the % chance of this trigger being fired at each clock tick in which all of the trigger conditions are met");
		...
		textTextField.setToolTipText("Enter the text to be shown in an employee's overhead bubble when this trigger is fired");
		...
		else if(triggerTypeList.getSelectedItem().equals(ActionTypeTrigger.RANDOM))
		{
			JSeparator separator1 = new JSeparator();
			separator1.setMaximumSize(new Dimension(450, 1));
			optionalPane.add(separator1);
		...
		for(int i=0; i<parts.size(); i++)
		{
			...
			if(tempPart.getSimSEObjectTypeType() == SimSEObjectTypeTypes.EMPLOYEE)
				//|| (tempPart.getSimSEObjectTypeType() == SimSEObjectTypeTypes.CUSTOMER)) // no overhead text for customers yet!
			{
	...
	public void actionPerformed(ActionEvent evt) // handles user actions
	{
		...
		{
			if(triggerTypeList.getSelectedItem().equals(ActionTypeTrigger.AUTO)) // autonomous trigger type selected
			{
				...
				pack();
				repaint();
				trigger = trigger.morph(ActionTypeTrigger.AUTO); // morph trigger to autonomous trigger
			}
			else if(triggerTypeList.getSelectedItem().equals(ActionTypeTrigger.USER)) // user trigger type selected
			{
				optionalPane.removeAll();
				optionalPane.add(menuTextPane);
				validate();
				pack();
				repaint();
				trigger = trigger.morph(ActionTypeTrigger.USER); // morph trigger to user trigger				
			}
			else if(triggerTypeList.getSelectedItem().equals(ActionTypeTrigger.RANDOM)) // random trigger type selected
			{
				optionalPane.removeAll();
				optionalPane.add(freqPane);
				validate();
				pack();
				repaint();
				trigger = trigger.morph(ActionTypeTrigger.RANDOM); // morph trigger to random trigger				
			}
		}
		...
		else if(source == viewEditButton)
		{
			if(participantList.isSelectionEmpty() == false) // a participant is selected
			{
				...
				ActionTypeParticipantTrigger partTrig = trigger.getParticipantTrigger((String)(participantList.getSelectedValue()));
				ActionTypeParticipantTriggerConstraintsInfoForm constsForm =
					new ActionTypeParticipantTriggerConstraintsInfoForm(this, partTrig, trigger);
				if(trigger.isGameEndingTrigger())
				{
					...
					if(attSetToScoring)	
					{
						...
						{
							ActionTypeParticipantTrigger pTrig = (ActionTypeParticipantTrigger)partTrigs.elementAt(i);
							if(pTrig != partTrig) // not participant trigger that you just edited
							{
		...
		else if(source == okButton)
		{
			...
			if(errors.size() == 0) // input valid
			{
				...
				if(triggerTypeList.getSelectedItem().equals(ActionTypeTrigger.USER)) // user trigger
				{
					((UserActionTypeTrigger)(trigger)).setMenuText(menuTextField.getText()); // set the trigger's menu text
				}
				else if(triggerTypeList.getSelectedItem().equals(ActionTypeTrigger.RANDOM)) // random trigger
				{
					try
					{
						((RandomActionTypeTrigger)(trigger)).setFrequency(Double.parseDouble(frequencyTextField.getText())); // set the
						// trigger's frequency
					}
				...
				int index = actionInFocus.removeTrigger(originalTrigger.getName()); // remove old trigger
				if(index > -1) // trigger was previously there
				{
					actionInFocus.addTrigger(trigger, index); // add the new trigger in its old position
				}
	...
	private void initializeForm()
	{
		...
		if((trigger.getTriggerText() != null) && (trigger.getTriggerText().length() > 0))
		{
		...
		if(trigger instanceof AutonomousActionTypeTrigger) // trigger is autonomous type
		{
			triggerTypeList.setSelectedItem(ActionTypeTrigger.AUTO); // set type list to reflect this
		}
		else if(trigger instanceof UserActionTypeTrigger) // trigger is user type
		{
			triggerTypeList.setSelectedItem(ActionTypeTrigger.USER); // set type list to reflect this
			menuTextField.setText(((UserActionTypeTrigger)(trigger)).getMenuText()); // set menu text field to reflect actual value
		}
		else if(trigger instanceof RandomActionTypeTrigger) // trigger is random type
		{
			triggerTypeList.setSelectedItem(ActionTypeTrigger.RANDOM); // set type list to reflect this
			frequencyTextField.setText((new Double(((RandomActionTypeTrigger)(trigger)).getFrequency())).toString()); // set frequency
	...
	private Vector inputValid()
	{
		...
		{
				messages.add("Please enter text for the trigger's name -- between 2 and 40 alphabetic characters");	// warn user to enter a valid 
				// name			
		}
		...
		for(int i=0; i<cArrayA.length; i++)
		{
			...
			{
				messages.add("Please enter only alphabetic characters for the trigger's name");	// warn user to enter a valid 
				// name	
			}
		}		
		...
		{
			ActionTypeTrigger tempTrig = ((ActionTypeTrigger)existingTrigs.elementAt(i));
			if((tempTrig.getName().equalsIgnoreCase(nameField.getText())) && (tempTrig != originalTrigger)) // name entered is not unique 
				// (there is already another trigger defined with the same name (hence, invalid)
			{
		...
		{
			if(trigText.equalsIgnoreCase("Everyone stop what you're doing")) // reserved phrase
			{
				messages.add("\"Everyone stop what you're doing\" is a reserved phrase");
			}
		}	
		
		// If this is a game-ending trigger, check that there is a scoring attribute assigned:
		...
		if(gameEndCBox.isSelected()) // game-ending trigger
		{
			...
			{
				messages.add("Since this is a game-ending trigger, you must assign exactly one attribute to be the scoring attribute");
			}
		}
		
		if(((String)(triggerTypeList.getSelectedItem())).equals(ActionTypeTrigger.USER)) // user trigger
		{
			...
			for(int i=0; i<existingActionTypes.size(); i++)
			{
				...
				if(tempAct.getName().equals(actionInFocus.getName()) == false) // not this action
				{
					...
					for(int j=0; j<tempTrigs.size(); j++)
					{
						...
						if((tempTrig instanceof UserActionTypeTrigger) && (tempTrig != originalTrigger)) // only check user
					...
					for(int j=0; j<tempDests.size(); j++)
					{
						...
						if(tempDest instanceof UserActionTypeDestroyer) // only check user destroyers
						{
			...
			for(int i=0; i<allTrigs.size(); i++)
			{
				...
				if((tempTrig instanceof UserActionTypeTrigger) && (tempTrig != originalTrigger))
				{
					if(((UserActionTypeTrigger)(tempTrig)).getMenuText().equals(menuTextField.getText())) // there already exists
						// a trigger with this menu text
					{
		...
		else if(((String)(triggerTypeList.getSelectedItem())).equals(ActionTypeTrigger.RANDOM)) // random trigger
		{
			...
			if((freqInput != null) && (freqInput.length() > 0)) // field is not blank
			{
				try
				{
					...
					if((freq < 0) || (freq > 100))
					{
						messages.add("Frequency must be a valid real number between 0 and 100");
					}
				}
				catch(NumberFormatException e)
				{
