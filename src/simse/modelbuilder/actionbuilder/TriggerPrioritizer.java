/* This class defines the window through which triggers can be put in order for priority of execution*/
...
public class TriggerPrioritizer extends JDialog implements ActionListener
{
	...
	private Vector nonPrioritizedTriggers; // vector of triggers that haven't been prioritized yet
	private Vector prioritizedTriggers; // vector of triggers that have been prioritized
	...
	public TriggerPrioritizer(JFrame owner, DefinedActionTypes acts)
	{
		...
		for(int i=0; i<allActions.size(); i++)
		{
			...
			for(int j=0; j<allTrigs.size(); j++)
			{
				...
				String triggerInfo = new String(trig.getName() + " - " + tempAct.getName() + " (");
				...
				{
					if(prioritizedTriggers.size() == 0) // no elements have been added yet to the prioritized trigger list
					{
					...
					else
					{
						...
						for(int k=0; k<prioritizedTriggers.size(); k++)
						{
							...
							String tempTrigName = tempTrigInfo.substring(0, tempTrigInfo.indexOf(' '));
							String tempActName = tempTrigInfo.substring((tempTrigInfo.indexOf('-') + 2), (tempTrigInfo.indexOf('(') - 1));
							...
							{
								prioritizedTriggers.insertElementAt(triggerInfo, k); // insert the trigger
								break;
							}
							else if(k == (prioritizedTriggers.size() - 1)) // on the last element
							{
								prioritizedTriggers.add(triggerInfo); // add the trigger to the end of the list
								break;
							}
						}
					}
		...
		nonPrioritizedTriggerList.setVisibleRowCount(10); // make 10 items visible at a time
		nonPrioritizedTriggerList.setSelectionMode(ListSelectionModel.SINGLE_SELECTION); // only allow the user to select one item at a time
		prioritizedTriggerList = new JList(prioritizedTriggers);
		prioritizedTriggerList.setVisibleRowCount(10); // make 10 items visible at a time
		prioritizedTriggerList.setSelectionMode(ListSelectionModel.SINGLE_SELECTION); // only allow the user to select one item at a time
	...
	public void actionPerformed(ActionEvent evt) // handles user actions
	{
		...
		else if(source == rightArrowButton)
		{
			...
			nonPrioritizedTriggers.remove(selIndex); // remove trigger from lefthand list
			prioritizedTriggers.add(tempStr); // add to righthand list
			String tempSelectedVal = ((String)(prioritizedTriggerList.getSelectedValue()));
			// reset list data:
			...
			if((nonPrioritizedTriggers.size() == 0) || (nonPrioritizedTriggerList.getSelectedIndex() > (nonPrioritizedTriggers.size() - 1))) // no
				// item is selected (had to use nonPrioritizedTriggers.size() - 1 because it doesn't work using selected index = -1 or
				// selected value == null!
			{
				// disable button:
				rightArrowButton.setEnabled(false);
		...
		else if(source == leftArrowButton)
		{
			...
			String tempStr = (String)(prioritizedTriggers.elementAt(selIndex)); // remove trigger from righthand list
			prioritizedTriggers.remove(selIndex); // remove trigger from righthand list
			nonPrioritizedTriggers.add(tempStr); // add to lefthand list
			String tempSelectedVal = ((String)(nonPrioritizedTriggerList.getSelectedValue()));			
			// reset list data:
			...
			if((prioritizedTriggers.size() == 0) || (prioritizedTriggerList.getSelectedIndex() > (prioritizedTriggers.size() - 1))) // no
				// item is selected (had to use prioritizedTriggers.size() - 1 because it doesn't work using selected index = -1 or
				// selected value == null!
			{
				// disable butons:
				leftArrowButton.setEnabled(false);
		...
		else if(source == moveUpButton)
		{
			...
			{
				String tempAct = (String)(prioritizedTriggers.remove(selIndex)); // remove trigger
				prioritizedTriggers.insertElementAt(tempAct, (selIndex - 1)); // move up
				prioritizedTriggerList.setSelectedIndex(selIndex - 1);
				prioritizedTriggerList.repaint();
				if((prioritizedTriggers.size() == 0) || (prioritizedTriggerList.isSelectionEmpty())) // no item is selected
				{
					// disable butons:
					leftArrowButton.setEnabled(false);
					moveUpButton.setEnabled(false);
		...
		{
			int selIndex = prioritizedTriggerList.getSelectedIndex();
			if(selIndex < (prioritizedTriggers.size() - 1)) // last list element wasn't chosen
			{
				String tempAct = (String)(prioritizedTriggers.remove(selIndex)); // remove trigger
				prioritizedTriggers.insertElementAt(tempAct, (selIndex + 1)); // move down
				prioritizedTriggerList.setSelectedIndex(selIndex + 1);
				prioritizedTriggerList.repaint();
				if((prioritizedTriggers.size() == 0) || (prioritizedTriggerList.isSelectionEmpty())) // no item is selected
				{
