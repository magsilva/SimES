/* This class defines a form for entering info about trigger constraints on an action participant */
...
{
	private ActionTypeTrigger outerTrigger; // outer trigger that this participant trigger is attached to
	...
	public ActionTypeParticipantTriggerConstraintsInfoForm(JDialog owner, ActionTypeParticipantTrigger trig, ActionTypeTrigger outerTrig)
	{
		super(owner, true);
		actualTrigger = trig;
		triggerInFocus = (ActionTypeParticipantTrigger)(actualTrigger.clone());
		participantInFocus = trig.getParticipant();
		outerTrigger = outerTrig;
		...
		for(int i=0; i<attributes.size(); i++)
		{
			...
			if(outerTrigger.isGameEndingTrigger())
			{
				...
				if(tempButt.isSelected())
				{
					...
					Vector partConsts = triggerInFocus.getAllConstraints();
