/* This class defines a form for entering info about destroyer constraints on an action participant */
...
public class ActionTypeParticipantDestroyerConstraintsInfoForm extends JDialog implements ActionListener
{
	private ActionTypeDestroyer outerDestroyer; // outer destroyer that this participant destroyer is attached to
	private ActionTypeParticipantDestroyer destroyerInFocus; // temporary copy of destroyer whose values are being edited
	...
	private ActionTypeParticipantConstraint constraintInFocus; // particular constraint of the destroyer being edited
	private Vector attributes; // Attributes of the constraint currently in focus
	private Vector radioButts; // radio buttons to indicate whether or not the attribute corresponds to the score	
	...
	public ActionTypeParticipantDestroyerConstraintsInfoForm(JDialog owner, ActionTypeParticipantDestroyer dest, 
		ActionTypeDestroyer outerDest)
	{
		super(owner, true);
		actualDestroyer = dest;
		destroyerInFocus = (ActionTypeParticipantDestroyer)(actualDestroyer.clone());
		...
		JScrollPane middlePaneMain = new JScrollPane(middlePane, JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED, 
	...
	public void actionPerformed(ActionEvent evt) // handles user actions
	{
		...
		if(source == okButton) // okButton has been pressed
		{
			...
			if(errors.size() == 0) // input valid
			{
				...
				// set the values to the actual destroyer from the copy whose values have been edited:
		...
		for(int i=0; i<attributes.size(); i++)
		{
			...
			if(outerDestroyer.isGameEndingDestroyer())
			{
				...
				if(tempButt.isSelected())
				{
					...
					{
						ActionTypeParticipantConstraint partConst = (ActionTypeParticipantConstraint)partConsts.elementAt(j);
	...
	private void setConstraintInFocus()
	{
		...
		constraintInFocus = destroyerInFocus.getConstraint((String)(typeList.getSelectedValue()));
		// set title:
		...
		for(int i=0; i<attributes.size(); i++)
		{
			...
			if(outerDestroyer.isGameEndingDestroyer())
			{
				...
				rButt.setSelected(constraintInFocus.getAttributeConstraint(att.getName()).isScoringAttribute());
