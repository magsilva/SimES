...
public class ActionBuilderGUI extends JPanel implements ActionListener
{
	...
	private JButton addParticipantButton; // button for adding a participant to an action
	private JButton editParticipantButton; // button for editing a participant in an action
	private JButton removeParticipantButton; // button for deleting a participant from an action
	...
	public void actionPerformed(ActionEvent evt) // handles user actions
	{
		...
		if(source == createNewActionButton) // User has pressed the "Create New Action" button
		{
			createNewAction();
		}

		else if(source == addParticipantButton) // user has requested to add a new participant
		{
			addParticipant();
			editParticipantButton.setEnabled(false);
			removeParticipantButton.setEnabled(false);
		...
		{
			ActionTypeTriggerManagementForm trigForm = new ActionTypeTriggerManagementForm(mainGUI, actTblMod.getActionTypeInFocus(), actions);
	...
	{
		String response = JOptionPane.showInputDialog(null, "Enter a name for this action:", "Enter Action Name",
		...
		if(response != null)
		{
			...
			{
				ActionType newAction = new ActionType(response); // create new action type
	...
	private boolean actionNameInputValid(String input) // returns true if input is a valid action name, false if not
	{
		char[] cArray = input.toCharArray();

		// Check for length constraints:
		if((cArray.length < 2) || (cArray.length > 40)) // user has a string shorter than 2 chars or longer than 40 chars
		{
			return false;
		}
		...
		for(int i=0; i<existingActionTypes.size(); i++)
		{
			...
			if(tempAct.getName().equalsIgnoreCase(input)) // name entered is not unique (there is already another action defined with the
	...
	private void addParticipant()
	{
		...
		apInfoForm = new ActionTypeParticipantInfoForm(mainGUI, actTblMod.getActionTypeInFocus(), null, objects);
	...
	{
		actTblMod.refreshData();
		apInfoForm = new ActionTypeParticipantInfoForm(mainGUI, actTblMod.getActionTypeInFocus(), p, objects);
	...
	private void removeParticipant(ActionTypeParticipant p)
	{
		int choice = JOptionPane.showConfirmDialog(null, ("Really remove " + p.getName() + " participant?"),
			"Confirm Participant Removal", JOptionPane.YES_NO_OPTION);
	...
	private void setupActionTableSelectionListenerStuff() // enables edit and remove participant buttons whenever a row (participant)
		// is selected
	{
	...
	private void setupDefinedActionsListSelectionListenerStuff() // enables view/edit button whenever a list item (action) is selected
	{
		// Copied from a Java tutorial:
	...
	{
		actTblMod.setActionTypeInFocus(newAct); // set focus of action table to new action type
		actionTableTitle.setText((newAct.getName()) + " Action Participants:"); // change title of table to reflect new action
		// enable buttons:
	...
	private void removeAction(String actionName) // removes the action with the specified name from the data structure
	{
		int choice = JOptionPane.showConfirmDialog(null, ("Really remove " + actionName + " action type?"),
			"Confirm Action Type Removal", JOptionPane.YES_NO_OPTION);
		if(choice == JOptionPane.YES_OPTION)
		{
			if((actTblMod.getActionTypeInFocus() != null) && (actTblMod.getActionTypeInFocus().getName() == actionName)) // removing
