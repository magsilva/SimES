...
public class ActionTypeParticipantInfoForm extends JDialog implements ActionListener
{
	private ActionType actionInFocus; // original action type whose participant is being edited
	...
	private ActionTypeParticipant participant; // copy of participant whose values are being edited
	private ActionTypeParticipant originalParticipant; // participant (as it was upon entry to this constructor) whose values are being
		// edited
	private DefinedObjectTypes objects; // data structure for holding all of the defined SimSE object types
	private boolean newParticipant = true; // whether or not this is a newly created participant (true) or one being edited (false)

	private JTextField nameField; // for entering a name for the participant
	private JComboBox guardList; // for choosing a guard
	private JTextField quantityTextField; // for entering the quantity of the participant
	private JTextField maxValTextField; // for entering the max value of the quantity of the participant
	private JComboBox metaTypeList; // for choosing the meta-type
	private Vector checkBoxes; // vector of JCheckBoxes, each one representing a type
	...
	public ActionTypeParticipantInfoForm(JFrame owner, ActionType action, ActionTypeParticipant part, DefinedObjectTypes objs)
	{
		super(owner, true);
		...
		if(guardList.getSelectedItem() != Guard.getText(Guard.AT_LEAST_AND_AT_MOST))
		{
			maxValTextField.setEnabled(false); // this field doesn't apply unless the guard is "at least/at most"
		}
		...
		JLabel instructionsLabel = new JLabel("That are of type (check all that apply):");
		...
		Vector objTypes = objects.getAllObjectTypesOfType(SimSEObjectTypeTypes.getIntRepresentation((String)(metaTypeList.getSelectedItem())));
		...
		for(int i=0; i<objTypes.size(); i++)
		{
			...
			JCheckBox tempCheckBox = new JCheckBox(((SimSEObjectType)(objTypes.elementAt(i))).getName());
		...
		else // new participant
		{
			...
			participant = new ActionTypeParticipant(SimSEObjectTypeTypes.getIntRepresentation((String)(metaTypeList.getSelectedItem())));
			
			for(int i=0; i<triggers.size(); i++)
			{
				ActionTypeTrigger trig = (ActionTypeTrigger)triggers.elementAt(i);
				trig.addParticipantTrigger(new ActionTypeParticipantTrigger(participant));
			}
			for(int i=0; i<destroyers.size(); i++)
			{
				ActionTypeDestroyer dest = (ActionTypeDestroyer)destroyers.elementAt(i);
				dest.addParticipantDestroyer(new ActionTypeParticipantDestroyer(participant));
			}
		}
	...
	public void actionPerformed(ActionEvent evt) // handles user actions
	{
		...
		{
			int selectedGuard = Guard.getIntRepresentation((String)guardList.getSelectedItem());
		...
		{
			if(SimSEObjectTypeTypes.getIntRepresentation((String)(metaTypeList.getSelectedItem()))
				!= participant.getSimSEObjectTypeType()) // a new type is selected (not the type that this participant already was)
				{
					participant.setSimSEObjectTypeType(SimSEObjectTypeTypes.getIntRepresentation((String)(metaTypeList.getSelectedItem())));
					refreshCheckBoxes();
				}
		}

		else if(source instanceof JCheckBox) // one of the check boxes have been checked or unchecked
		{
			// get the text of the check box:
			String typeName = ((JCheckBox)(source)).getText();
			if(((JCheckBox)source).isSelected()) // a check box has just been selected
			{
				// add this object type to the participant:
				participant.addSimSEObjectType(objects.getObjectType(SimSEObjectTypeTypes.getIntRepresentation((String)(metaTypeList.getSelectedItem())),
					typeName));
				// add a new empty constraint to the participant's triggers:
				for(int i=0; i<triggers.size(); i++)
				{
					ActionTypeTrigger trig = (ActionTypeTrigger)triggers.elementAt(i);
					trig.getParticipantTrigger(participant.getName()).addEmptyConstraint(objects.getObjectType(SimSEObjectTypeTypes.getIntRepresentation((String)(metaTypeList.getSelectedItem())),
					typeName));
				}
				...
				{
					ActionTypeDestroyer dest = (ActionTypeDestroyer)destroyers.elementAt(i);
					dest.getParticipantDestroyer(participant.getName()).addEmptyConstraint(objects.getObjectType(SimSEObjectTypeTypes.getIntRepresentation((String)(metaTypeList.getSelectedItem())),
					typeName));
				}
			}
			else // a check box has just been de-selected
			{
				...
				{
					ActionTypeTrigger trig = (ActionTypeTrigger)triggers.elementAt(i);
					trig.getParticipantTrigger(participant.getName()).removeConstraint(typeName);
				}
				// remove the constraint from the participant's destroyer:
				for(int i=0; i<destroyers.size(); i++)
				{
					ActionTypeDestroyer dest = (ActionTypeDestroyer)destroyers.elementAt(i);					
					dest.getParticipantDestroyer(participant.getName()).removeConstraint(typeName);
				}
			}
		}
	...
	private void initializeForm() // initializes the GUI components to reflect the participant being edited
	{
		// fill in values:
		...
		// guard:
		guardList.setSelectedItem(Guard.getText(participant.getQuantity().getGuard()));

		// quantity:
		if(participant.getQuantity().isQuantityBoundless() == false)
		{
			quantityTextField.setText(participant.getQuantity().getQuantity()[0].toString());
		}
		if(participant.getQuantity().getGuard() == Guard.AT_LEAST_AND_AT_MOST)
		{
			...
			{
				maxValTextField.setText(participant.getQuantity().getQuantity()[1].toString());
			}
		}

		// meta-type:
		metaTypeList.setSelectedItem(SimSEObjectTypeTypes.getText(participant.getSimSEObjectTypeType()));
	...
	private Vector inputValid() // validates the input from the fields; returns null if input is valid,
		// or a Vector of String messages explaining the error(s)
	{
		...
		if((nameInput.equalsIgnoreCase(SimSEObjectTypeTypes.getText(SimSEObjectTypeTypes.EMPLOYEE)))
			|| (nameInput.equalsIgnoreCase(SimSEObjectTypeTypes.getText(SimSEObjectTypeTypes.ARTIFACT)))
				|| (nameInput.equalsIgnoreCase(SimSEObjectTypeTypes.getText(SimSEObjectTypeTypes.TOOL)))
					|| (nameInput.equalsIgnoreCase(SimSEObjectTypeTypes.getText(SimSEObjectTypeTypes.CUSTOMER)))
						|| (nameInput.equalsIgnoreCase(SimSEObjectTypeTypes.getText(SimSEObjectTypeTypes.PROJECT)))
							|| (nameInput.equalsIgnoreCase("action")) || (nameInput.equalsIgnoreCase("compiler"))) // System-wide keywords
		{
		...
		if((quantityInput != null) && (quantityInput.length() > 0)) // field is not blank
		{
			try
			{
				int quantity = Integer.parseInt(quantityInput); // parse the string into an int
				if(quantity < 0)
				{
		...
		if((maxValInput != null) && (maxValInput.length() > 0)) // field is not blank
		{
			try
			{
				int maxVal = Integer.parseInt(maxValInput); // parse the string into an int
				if(maxVal < 0)
				{
		...
		if((quantityInput != null) && (quantityInput.length() > 0) && (maxValInput != null) && (maxValInput.length() > 0)) // neither
			// field is blank
		{
			try
			{
				...
				{
					messages.add("'At most' value must be greater than or equal to 'at least' value");
				}
			}
			catch(NumberFormatException e)
			{
				System.out.println(e.getMessage());
				// Don't actually add a message to messages here because if either of these fields are not valid positive integers, it
	...
	private void addEditParticipant() // adds/edits the participant in focus
	{
		...
		// set guard:
		participant.getQuantity().setGuard(Guard.getIntRepresentation((String)(guardList.getSelectedItem())));
		...
		else // quantity text field non-empty
		{
			try
			{
				newQuantity[0] = new Integer(Integer.parseInt(quantityTextField.getText()));
			}
			catch(NumberFormatException e)
			{
				// Note: this exception should never be thrown because inputValid() should have already been called on this
					// input
				System.out.println(e.getMessage());
			}
		}
		...
		else // max val text field non-empty
		{
			try
			{
				newQuantity[1] = new Integer(Integer.parseInt(maxValTextField.getText()));
			}
			catch(NumberFormatException e)
			{
				// Note: this exception should never be thrown because inputValid() should have already been called on this
		...
		{
			int indexOfPart = actionInFocus.getAllParticipants().indexOf(originalParticipant);
			actionInFocus.removeParticipant(originalParticipant); // remove the old version
			//triggerInFocus.getParticipantTrigger(originalParticipant.getName()).setParticipant(participant); // set the participant for
				// the participant trigger to the copy participant that's been edited
			String name = originalParticipant.getName();
			for(int i=0; i<triggers.size(); i++)
			{
				ActionTypeTrigger trig = (ActionTypeTrigger)triggers.elementAt(i);
				trig.getParticipantTrigger(name).setParticipant(participant); // set the participant for the participant trigger to the copy 
					// participant that's been edited
			}
			for(int i=0; i<destroyers.size(); i++)
			{
				ActionTypeDestroyer dest = (ActionTypeDestroyer)destroyers.elementAt(i);
				dest.getParticipantDestroyer(name).setParticipant(participant); // set the participant for the participant destroyer to the copy
					// participant that's been edited
			}
			if(indexOfPart != -1) // participant was in the action
			{
				actionInFocus.addParticipant(participant, indexOfPart); // add the edited participant at its old position
			}
		}
		else // new participant
		{
	...
	private boolean participantNameIsUnique() // returns true if it is unique, false otherwise, and also takes care of asking the user
		// if they want to overwrite it (and overwriting it, if desired)
	{
		String nameInput = nameField.getText();
		if((newParticipant) || ((!newParticipant) && (participant.getName().equals(nameInput) == false))) // only perform this check if
			// this is a newly created participant, or it is an edited participant and the name has been changed
		{
			Vector existingParticipants = actionInFocus.getAllParticipants();
			for(int i=0; i<existingParticipants.size(); i++)
			{
				ActionTypeParticipant tempPart = ((ActionTypeParticipant)existingParticipants.elementAt(i));
				if(tempPart.getName().equalsIgnoreCase(nameInput)) // name entered is not unique (there is already another participant of
					// this action defined with the same name
				{
					int choice = JOptionPane.showConfirmDialog(null, ("Previously defined " + tempPart.getName() +
						" participant will be overwritten. Continue?"), "Warning", JOptionPane.YES_NO_OPTION);
					if(choice == JOptionPane.YES_OPTION)
					{
						if(!newParticipant) // this is a participant being edited, not a new one
						{
	...
	private void refreshCheckBoxes() // refreshes the check boxes to reflect the meta-type chosen in the meta-type list
	{
		Vector objTypes = objects.getAllObjectTypesOfType(SimSEObjectTypeTypes.getIntRepresentation((String)(metaTypeList.getSelectedItem())));
		checkBoxes.clear();
	...
	private void setUpToolTips()
	{
		...
		{
			quantityTextField.setToolTipText("Enter quantity (leave blank if boundless)");
