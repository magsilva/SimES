...
public class ActionTypeParticipantConstraint implements Cloneable {
  SimSEObjectType objType; // pointer to the SimSEObjectType that this
                           // constraint is on
  ActionTypeParticipantAttributeConstraint[] constraints; // constraints on this
                                                          // SimSEObjectType's
                                                          // attributes
  ...
  public ActionTypeParticipantConstraint(SimSEObjectType type) {
    ...
    for (int i = 0; i < constraints.length; i++) {
      constraints[i] = new ActionTypeParticipantAttributeConstraint(
  ...
  public Object clone() {
    try {
      ActionTypeParticipantConstraint cl = (ActionTypeParticipantConstraint) (super
          .clone());
      ...
      for (int i = 0; i < constraints.length; i++) {
        clonedConsts[i] = (ActionTypeParticipantAttributeConstraint) (constraints[i]
            .clone());
      }
  ...
  public SimSEObjectType getSimSEObjectType() // returns a COPY of the
                                              // SimSEObjectType
  {
  ...
  public void addAttributeConstraint(
      ActionTypeParticipantAttributeConstraint newConstraint) // replaces
                                                              // existing
                                                              // constraint for
                                                              // the
  ...
  public ActionTypeParticipantAttributeConstraint getAttributeConstraint(
      String name) // returns the constraint on the attribute with
  // the specified name
  {
    for (int i = 0; i < constraints.length; i++) {
