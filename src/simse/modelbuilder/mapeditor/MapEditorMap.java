...
public class MapEditorMap extends SimSEMap
{
	...
	public void paintComponent(Graphics g)
	{
		for (int i = 0; i < MapData.Y_MAPSIZE; i++)
		{
			for (int  j = 0; j < MapData.X_MAPSIZE; j++)
			{
				g.drawImage(mapRep[j][i].getBase(), j * MapData.TILE_SIZE, i * MapData.TILE_SIZE, this);
...
//System.out.println("MEMAP" + i);
					...
					g.drawRect(tmp.getXLocation() * MapData.TILE_SIZE, tmp.getYLocation() * MapData.TILE_SIZE,MapData.TILE_SIZE,MapData.TILE_SIZE);
	...
	public void popupMenuActions(JMenuItem source)
	{
		...
		else if (source.getText().startsWith("- Employee"))
		{
			...
			for (int i = 0; i < sopUsers.size(); i++)
			{
				...
				{
					JOptionPane.showMessageDialog(null,tmp.getName() + " currently resides on this tile, please choose another.");
					i = sopUsers.size();
					tileIsOccupied = true;
				}
