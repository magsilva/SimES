...
public class SimSEMap extends JPanel implements MouseListener, ActionListener {
  ...
  public SimSEMap(DefinedObjectTypes objTypes, CreatedObjects objs,
      DefinedActionTypes acts, File iconDir, Hashtable startStateObjs,
      Hashtable ruleObjs) {
  ...
  public Vector loadFile(File inputFile, File iconDir) // loads the map from the
                                                       // input file (.mdl) and
                                                       // returns a Vector of
                                                       // warning messages
  {
    ...
    while (ssObj.hasMoreElements()) {
      ...
      String tmpIconLoc = iconDirectory.getPath() + "\\"
          + (String) ssImg.nextElement();
      ...
      if (objType.equals(SimSEObjectTypeTypes
        ...
        UserData tmpUser = new UserData(simObj, tmpIconLoc, this, false, hired,
            -1, -1);
    ...
    while (rObj.hasMoreElements()) {
      ...
      if (objType.equals(SimSEObjectTypeTypes
          .getText(SimSEObjectTypeTypes.EMPLOYEE))) {
        UserData tmpUser = new UserData(simObj, tmpIconLoc, this, false, false,
            -1, -1);
        sopUsers.add(tmpUser);
      }
    }
          ...
          while (!endOfMap) {
            ...
            } else // not end of map yet
            {
              if (currentLine.equals("<beginSOPUsers>")) {
                ...
                while (!currentLine.equals("<endSOPUsers>")) // read in the sop
                                                             // users:
                {
