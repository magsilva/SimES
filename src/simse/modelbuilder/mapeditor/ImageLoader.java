    ...
    public static void copyImagesToDir(String dir)
    {
        try
        {
            ...
            while (true)
            {
                ...
                // makes the temp files in the images directory if images dir is non existant
                if (tmp.length() > 0)
                    new File(dir+ze.getName()).createNewFile();
                
                byte[] buffer = new byte[1024];
                int len = 1024;
                BufferedOutputStream out = new BufferedOutputStream(new FileOutputStream(dir+ze.getName()));
