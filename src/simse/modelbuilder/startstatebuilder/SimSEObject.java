...
public class SimSEObject implements Cloneable
{
	...
	public Object clone()
	{
		try
		{
			...
			{
				clonedAtts.add((InstantiatedAttribute)(((InstantiatedAttribute)(instantiatedAttributes.elementAt(i))).clone()));
			}
	...
	public InstantiatedAttribute getKey() // returns the instantiated attribute that is the "key" of this object -- the attribute that must
		// be unique
	{
		for(int i=0; i<instantiatedAttributes.size(); i++)
		{
			InstantiatedAttribute tempAttr = (InstantiatedAttribute)(instantiatedAttributes.elementAt(i));
	...
	public int getNumAttributes() // returns the number of attributes this object has
	{
		return instantiatedAttributes.size();
	}


	public InstantiatedAttribute getAttribute(String name) // returns the instanatiated attribute with the specified name
	{
		for(int i=0; i<instantiatedAttributes.size(); i++)
		{
			InstantiatedAttribute a = (InstantiatedAttribute)instantiatedAttributes.elementAt(i);
	...
	public void addAttribute(InstantiatedAttribute att) // adds the instantiated attribute to this object
	{
		if(!hasAttribute(att.getAttribute())) // doesn't already have this attribute
		{
	...
	public boolean hasAttribute(Attribute att) // returns true if this object has the specified attribute as an instantiated
		// attribute, false otherwise
	{
		for(int i=0; i<instantiatedAttributes.size(); i++)
		{
			InstantiatedAttribute instAtt = (InstantiatedAttribute)instantiatedAttributes.elementAt(i);
			if((instAtt.getAttribute().getName().equals(att.getName())))// && (instAtt.getAttribute().getType() == att.getType()))
			{
				return true;
			}
		}
	...
	public void updateInstantiatedAttributes() // checks to make sure that each attribute in the object type has a corresponding instantiated
