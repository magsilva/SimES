/* This class is the data structure for holding the objects that are currently instantiated */
...
public class CreatedObjects
{
	...
	public Vector getAllObjectsOfType(SimSEObjectType type) // returns a Vector of SimSEObjects, all of them having the type specified
	{
		Vector v = new Vector();
	...
	public SimSEObject getObject(int type, String name, Object keyAttValue) // returns the object in the data structure with the specified type, name, and key attribute value
	{
		for(int i=0; i<objs.size(); i++)
		{
			SimSEObject tempObj = ((SimSEObject)objs.elementAt(i));
			if((tempObj.getSimSEObjectType().getType() == type) && (tempObj.getName().equals(name))
	...
	public void removeObject(int type, String name, Object keyAttValue) // removes the object w/ the specified name, type, and key attribute value from the data structure
	{
		for(int i=0; i<objs.size(); i++)
		{
			SimSEObject tempObj = ((SimSEObject)objs.elementAt(i));
	...
	public void updateAllInstantiatedAttributes() // calls updateAllInstantiatedAttributes() for all created objects
	{
		for(int i=0; i<objs.size(); i++)
		{
			SimSEObject tempObj = (SimSEObject)objs.elementAt(i);
