...
public class StartStateBuilderGUI extends JPanel implements ActionListener, ListSelectionListener, MouseListener
{
	...
	private JButton editStartingValButton; // button for editing the starting value of an attribute
	...
	public void actionPerformed(ActionEvent evt) // handles user actions
	{
		...
		if(source == okCreateObjectButton) // User has ok'ed the creation of a new object
		{
	...
	private void createObject(String selectedItem) // creates a new SimSE object of the type based on selectedItem and adds it to the data structure
	{
		...
		WindowFocusListener l = new WindowFocusListener ()
		{
			public void windowLostFocus (WindowEvent ev)
			{
				...
				{
					setObjectInFocus(attTblMod.getObjectInFocus()); // set newly created object to be the focus of the GUI
				}
				updateCreatedObjectsList();
	...
	private void setObjectInFocus(SimSEObject newObj) // sets the given object as the focus of this GUI
	{
		...
		attributeTableTitle.setText((newObj.getName()) + " " + (SimSEObjectTypeTypes.getText(newObj.getSimSEObjectType().getType()))
			+ " " + keyVal + " Attributes:"); // change title of table to reflect new object
		// disable buttons:
		editStartingValButton.setEnabled(false);
	}
	...
	private void updateCreatedObjectsList()
	{
		...
		for(int i=0; i<currentObjs.size(); i++)
		{
			...
			String type = SimSEObjectTypeTypes.getText(tempObj.getSimSEObjectType().getType());
			String keyVal = new String();
	...
	private void removeObject(SimSEObject obj) // removes this object from the data structure
	{
		...
		int choice = JOptionPane.showConfirmDialog(null, ("Really remove " + obj.getName() + " " +
		...
		if(choice == JOptionPane.YES_OPTION)
		{
			if((attTblMod.getObjectInFocus() != null) && (attTblMod.getObjectInFocus().getName() == obj.getName()) &&
				(attTblMod.getObjectInFocus().getSimSEObjectType().getType() == obj.getSimSEObjectType().getType())) // removing object
					// currently in focus
			{
				clearObjectInFocus(); // set it so that there's no object in focus
			}
			objects.removeObject(obj.getSimSEObjectType().getType(), obj.getName(), obj.getKey().getValue());
			removeObjectButton.setEnabled(false);
			updateCreatedObjectsList();
			((ModelBuilderGUI)mainGUI).setFileModSinceLastSave();
		}
	...
	private void resetCreateObjectList()
	{
		...
		for(int i=0; i<types.size(); i++) // go through all of the defined object types
		{
			...
			{
				String tempStr = new String(tempType.getName() + " " + SimSEObjectTypeTypes.getText(tempType.getType()));
