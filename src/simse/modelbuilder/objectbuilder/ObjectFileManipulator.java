/* This class is for generating the DefinedObjectTypes into a file and reading that file into memory*/
...
private boolean allowHireFire;
	...
	public void loadFile(File inputFile) // loads the mdl file into memory, filling the "objects" data structure with the data from the file
	{
						...
						else // not end of object types yet
						{
							if(currentLine.equals(BEGIN_OBJECT_TAG))
							{
								...
								while(!endOfObj)
								{
									...
									{
										endOfObj = true;
										objects.addObjectType(newObj); // add object type to defined object types
									}
									else if(currentLine.equals(BEGIN_ATTRIBUTE_TAG)) // beginning of attribute
									{
										...
										if(type == AttributeTypes.INTEGER) // integer attribute
										{
											...
											Integer minVal = null;
											if(minValStr.equals(BOUNDLESS) == false) // minimum value is not boundless
											{
												minVal = new Integer(minValStr);
											}
											String maxValStr = reader.readLine(); // get the maximum value
											Integer maxVal = null;
											if(maxValStr.equals(BOUNDLESS) == false) // maximum value is not boundless
											{
										...
										else if(type == AttributeTypes.DOUBLE) // double attribute
										{
											...
											String maxDigStr = reader.readLine(); // get the maximum value
											Integer maxDig = null;
										...
										{
											newObj.addAttribute(new NonNumericalAttribute(attName, type, visible, key, visibleAtEnd)); // add attribute to
											// object
										}
	  ...
	  {
			JOptionPane.showMessageDialog(null, ("Cannot find object file " + inputFile.getPath()), "File Not Found",
