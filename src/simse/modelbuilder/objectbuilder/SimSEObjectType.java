...
public class SimSEObjectType implements Cloneable
{
	...
	public Object clone()
	{
		try
		{
			...
			{
				clonedAtts.addElement((Attribute)(((Attribute)(attributes.elementAt(i))).clone()));
			}
	...
	public Attribute getKey() // returns the attribute that is the "key" of this object -- the attribute that must be unique
	{
		for(int i=0; i<attributes.size(); i++)
		{
	...
	public boolean hasKey() // returns true if this object has a key, false otherwise
	{
		for(int i=0; i<attributes.size(); i++)
		{
			Attribute tempAttr = (Attribute)(attributes.elementAt(i));
	...
	public void addAttribute(Attribute a, int index) // adds the attribute at the specified index
	{
		attributes.add(index, a);
	}


	public int removeAttribute(String name) // removes the attribute with the specified name and returns the position it removed it from
	{
		for(int i=0; i<attributes.size(); i++)
		{
	...
	public Attribute getAttribute(String name) // returns the attribute with the specified name
	{
		for(int i=0; i<attributes.size(); i++)
		{
			Attribute a = (Attribute)attributes.elementAt(i);
