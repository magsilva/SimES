/* This class defines the window through which information about an attribute can be added/edited */
...
public class AttributeInfoForm extends JDialog implements ActionListener
{
	...
	private JComboBox visibleAtEndList; // for choosing whether an attribute is visible at game end
	private JComboBox keyList; // for choosing whether or not an attribute is the key for the object type
	private JLabel minValLabel; // labels minimum value field
	private JLabel maxValLabel; // labels maximum value field
	private JTextField minValTextField; // for entering the minimum value of a variable
	private JTextField maxValTextField; // for entering the maximum value of a variable
	...
	public AttributeInfoForm(JFrame owner, SimSEObjectType object, Attribute attr)
	{
		...
		typeList.setBorder(BorderFactory.createEmptyBorder(0, 0, 0, 10)); // t, l, b, r
		...
		keyList.addItem("Yes");
		keyList.addItem("No");
		if(objectInFocus.getAllAttributes().size() == 0) // this is the first attribute being added
		{
	...
	public void actionPerformed(ActionEvent evt) // handles user actions
	{
		...
		if(source == typeList) // User has chosen a type
		{
			...
			if((selectedType == AttributeTypes.STRING) || (selectedType == AttributeTypes.BOOLEAN)) // non-numerical types
			{
		...
		else if(source == okButton) // okButton has been pressed
		{
			...
			String otherError = maxGreaterThanOrEqualToMin(); // check that max is greater than or equal to min
	...
	private void initializeForm() // initializes the text in the text fields (if any) and greys-out the min/max value fields if necessary
	{
		...
		{
			if((((NumericalAttribute)attribute).isMinBoundless()) == false) // min value is not boundless
			{
				minValTextField.setText(((NumericalAttribute)attribute).getMinValue().toString());
			}
			if((((NumericalAttribute)attribute).isMaxBoundless()) == false) // max value is not boundless
			{
				maxValTextField.setText(((NumericalAttribute)attribute).getMaxValue().toString());
			}
			...
			{
				if(((NumericalAttribute)attribute).getMinNumFractionDigits() != null) // has min num fraction digits
				{
					minNumDigitsTextField.setText(((NumericalAttribute)attribute).getMinNumFractionDigits().toString());
				}
		...
		// or a Vector of String messages explaining the error(s)
	{
		...
		for(int i=0; i<cArray.length; i++)
		{
			...
			{
				messages.add(new String("Name must consist of only alphabetic characters"));
				break;
			}
		}
		...
		if((attribute != null) && (attribute.isKey() == true) && (((String)keyList.getSelectedItem()) == "No")) // this is an attribute
			...
			// is no key for this object
		{
			if(objectInFocus.getNumAttributes() > 1) // this is not the only attribute
			{
				messages.add(new String("You must make a different attribute the key in order to remove this one as the key."));
			}
			else // this is the only attribute
			{
				messages.add(new String("You cannot remove this attribute as the key since it is the only attribute."));
			}
		}
		else if((objectInFocus.getNumAttributes() == 0) && (((String)keyList.getSelectedItem()) == "No")) // this is a new attribute being
			// created and it is also the first attribute being created for the object type in focus
		{
		...
		if(selectedType == AttributeTypes.DOUBLE) // type = double
		{
			...
			if((minValInput != null) && (minValInput.length() > 0)) // field is not blank
			{
				try
				{
					double minVal = Double.parseDouble(minValInput); // parse the string into a double
				}
			...
			if((maxValInput != null) && (maxValInput.length() > 0)) // field is not blank
			{
				try
				{
					double maxVal = Double.parseDouble(maxValInput); // parse the string into a double
				}
				catch(NumberFormatException e)
				{
			...
			if((minDigitsInput != null) && (minDigitsInput.length() > 0)) // field is not blank
			{
				try
				{
					int minDigits = Integer.parseInt(minDigitsInput); // parse the string into a int
					if((minDigits < 0) || (minDigits > 16))
					{
						messages.add(new String("Minimum number of fraction digits must be between 0 and 16"));
					}
				}
				catch(NumberFormatException e)
				{
			...
			if((maxDigitsInput != null) && (maxDigitsInput.length() > 0)) // field is not blank
			{
				try
				{
					int maxDigits = Integer.parseInt(maxDigitsInput); // parse the string into a int
					if((maxDigits < 0) || (maxDigits > 16))
					{
						messages.add(new String("Maximum number of fraction digits must be between 0 and 16"));
					}
				}
				catch(NumberFormatException e)
				{
		...
		else if(selectedType == AttributeTypes.INTEGER) // type = integer
		{
			...
			if((minValInput != null) && (minValInput.length() > 0)) // field is not blank
			{
				try
				{
					int minVal = Integer.parseInt(minValInput); // parse the string into an int
				}
				catch(NumberFormatException e)
				{
	...
	private String maxGreaterThanOrEqualToMin() // returns a string with the error message if this condition is not met, null otherwise
	{
		String minValInput = minValTextField.getText();
		String maxValInput = maxValTextField.getText();
		if((minValInput != null) && (minValInput.length() > 0) && (maxValInput != null) && (maxValInput.length() > 0)) // neither max nor min fields are blank
		{
			int selectedType = AttributeTypes.getIntRepresentation((String)typeList.getSelectedItem());
			if(selectedType == AttributeTypes.INTEGER)
			{
				try
				{
					int minVal = Integer.parseInt(minValInput); // parse the string into an integer
					int maxVal = Integer.parseInt(maxValInput); // parse the string into an integer
					if(maxVal < minVal) // invalid
					{
						return (new String("Maximum value must be greater than or equal to minimum value"));
					}
				}
				catch(NumberFormatException e)
				{
					System.out.println(e.getMessage());
					// since this has already been checked with call to inputValid(), this exception should never be thrown here.
				}
			}
		...
		// otherwise
	{
		String minDigitsInput = minNumDigitsTextField.getText();
		String maxDigitsInput = maxNumDigitsTextField.getText();
		if((minDigitsInput != null) && (minDigitsInput.length() > 0) && (maxDigitsInput != null) && (maxDigitsInput.length() > 0)) // neither
			// max nor min fields are blank
		{
			int selectedType = AttributeTypes.getIntRepresentation((String)typeList.getSelectedItem());
			if(selectedType == AttributeTypes.DOUBLE)
			{
				try
				{
					int minVal = Integer.parseInt(minDigitsInput); // parse the string into a int
					int maxVal = Integer.parseInt(maxDigitsInput); // parse the string into a int
					if(maxVal < minVal) // invalid
					{
						return (new String("Maximum number of fraction digits must be greater than or equal to minimum value"));
					}
				}
				catch(NumberFormatException e)
				{
	...
	{
		String nameInput = nameTextField.getText();
		if((attribute == null) || ((attribute != null) && (attribute.getName().equals(nameInput) == false))) // only perform this check if
			// this is a newly created attribute, or it is an edited attribute and the name has been changed
		{
			Vector existingAttributes = objectInFocus.getAllAttributes();
			for(int i=0; i<existingAttributes.size(); i++)
			{
				Attribute tempAttr = ((Attribute)existingAttributes.elementAt(i));
				...
				{
					int choice = JOptionPane.showConfirmDialog(null, ("Previously defined " + tempAttr.getName()
						+ " attribute will be overwritten. Continue?"), "Warning", JOptionPane.YES_NO_OPTION);
					if(choice == JOptionPane.YES_OPTION)
					{
						if(attribute != null) // this is an attribute being edited, not a new one
						{
							if((((String)keyList.getSelectedItem()) == "No") && (attribute.isKey() == true)) // this attribute being edited
								// was the key previously, but the user has just changed it to not be the key, hence there would be no key
								// for this object
							{
								if(objectInFocus.getNumAttributes() > 1) // this is not the only attribute
								{
									JOptionPane.showMessageDialog(null, "You must make a different attribute the key in order to remove this one as the key.",
										"Invalid Input", JOptionPane.ERROR_MESSAGE);
								}
								else // this is the only attribute
								{
									JOptionPane.showMessageDialog(null, "You cannot remove this attribute as the key since it is the only attribute.",
										"Invalid Input", JOptionPane.ERROR_MESSAGE);
								}
							}
							else // everything is okay!
							{
								...
								if(((String)keyList.getSelectedItem()) == "Yes") // this newly added/edited attribute is key
								{
									Attribute key = objectInFocus.getKey(); // get the current key
									if(keyAttName.equalsIgnoreCase(nameInput) == false) // newly added/edited attribute was not the key
										// previously
									{
						...
						else // this is a new attribute, not one being edited
						{
							...
							if(((String)keyList.getSelectedItem()) == "Yes") // this newly added/edited attribute is key
							{
								...
								{
									Attribute key = objectInFocus.getKey(); // get the current key
									if(key.getName().equalsIgnoreCase(nameInput) == false) // newly added/edited attribute was not the key
										// previously
									{
										key.setKey(false); // remove "key status" from previous key
									}
							...
							else if((((String)keyList.getSelectedItem()) == "No") && (tempAttr.isKey())) // this newly added attribute is
								// not the key, but the one it is supposed to be replacing was the key
							{
		...
		// function to ensure that you're adding a valid attribute
	{
		...
		if(((String)keyList.getSelectedItem()) == "Yes")
		{
			...
			if(objectInFocus.hasKey())
			{
				...
				if(keyAttr.getName().equals(nameTextField.getText()) == false) // newly added/edited attribute was not the key previously
				{
		...
		if((selectedType == AttributeTypes.DOUBLE) || (selectedType == AttributeTypes.INTEGER)) // numerical attribute
		{
			...
			if(selectedType == AttributeTypes.DOUBLE)
			{
				try
				{
					...
					else if((minValInput != null) && (minValInput.length() > 0)) // only max val field is blank
					{
						double minVal = Double.parseDouble(minValInput);
						min = new Double(minVal);
					}
					else if((maxValInput != null) && (maxValInput.length() > 0)) // only min val field is blank
					{
						double maxVal = Double.parseDouble(maxValInput);
						max = new Double(maxVal);
					}
			...
			else if(selectedType == AttributeTypes.INTEGER)
			{
				try
				{
					if((minValInput != null) && (minValInput.length() > 0) && (maxValInput != null) && (maxValInput.length() > 0)) // neither min val nor max val field is blank
					{
						int minVal = Integer.parseInt(minValInput);
						int maxVal = Integer.parseInt(maxValInput);
						// Add new attribute to object in focus:
						objectInFocus.addAttribute(new NumericalAttribute(nameTextField.getText(), selectedType, visible, key, visibleAtEnd,
		...
		{
			// Add new attribute to object in focus:
			objectInFocus.addAttribute(new NonNumericalAttribute(nameTextField.getText(), selectedType, visible, key, visibleAtEnd));
		}
		setVisible(false);
		dispose();
	}
		...
		// function to ensure that you're editing the attribute with valid values
	{
		...
		if(((String)visibleAtEndList.getSelectedItem()) == "Yes")
		{
			attribute.setVisibleOnCompletion(true); // set to new visible on completion status
		}
		else // "No" selected
		{
		...
		if(((String)keyList.getSelectedItem()) == "Yes")
		{
			if(objectInFocus.hasKey())
			{
				objectInFocus.getKey().setKey(false); // remove "key status" from previous key
			}
		...
		if((selectedType == AttributeTypes.DOUBLE) || (selectedType == AttributeTypes.INTEGER)) // numerical attribute
		{
			...
			if(selectedType == AttributeTypes.DOUBLE)
			{
				try
				{
						...
						// val nor max val field is blank
					{
						...
						((NumericalAttribute)attribute).setMinValue(new Double(Double.parseDouble(minValInput)));
						((NumericalAttribute)attribute).setMaxValue(new Double(Double.parseDouble(maxValInput)));
					}
					else if((minValInput != null) && (minValInput.length() > 0)) // only max val field is blank
					{
						...
						// neither min nor max field is blank
					{
						...
						((NumericalAttribute)attribute).setMinNumFractionDigits(new Integer(Integer.parseInt(minDigitsInput)));
						((NumericalAttribute)attribute).setMaxNumFractionDigits(new Integer(Integer.parseInt(maxDigitsInput)));
					}
					else if((minDigitsInput != null) && (minDigitsInput.length() > 0)) // only max field is blank
					{
						// Set min to new val:
			...
			else if(selectedType == AttributeTypes.INTEGER)
			{
				try
				{
					...
					{
						// Set min and max vals to new vals:
						((NumericalAttribute)attribute).setMinValue(new Integer(Integer.parseInt(minValInput)));
						((NumericalAttribute)attribute).setMaxValue(new Integer(Integer.parseInt(maxValInput)));
					}
					else if((minValInput != null) && (minValInput.length() > 0)) // only max val field is blank
					{
						// Set min val to new val:
		...
		{
			if((attribute.getType() == AttributeTypes.INTEGER) || (attribute.getType() == AttributeTypes.DOUBLE)) // attribute is changing from a numerical attribute
