...
public class AttributeInfoForm extends JDialog implements ActionListener {
  ...
  private void initializeForm() // initializes the text in the text fields (if
                                // any) and greys-out the min/max value fields
                                // if necessary
  {
    // fill in values:
    ...
    if ((attribute == null) || (attribute instanceof NonNumericalAttribute)) // min/max
                                                                             // val
                                                                             // fields
                                                                             // do
                                                                             // not
    ...
    } else // attribute is a numerical attribute
    {
      ...
      } else // double attribute
      {
        ...
        if (((NumericalAttribute) attribute).getMaxNumFractionDigits() != null) // has
                                                                                ...
                                                                                // digits
        {
          maxNumDigitsTextField.setText(((NumericalAttribute) attribute)
  ...
  private Vector inputValid() // validates the input from the text fields;
  ...
  // or a Vector of String messages explaining the error(s)
  {
    ...
    if ((attribute != null) && (attribute.isKey() == true)
        && (((String) keyList.getSelectedItem()) == "No")) // this is an
                                                           // attribute
    // being edited, not a new one, it was the key, but the user has just
    // changed it to not be the key, hence there
    // is no key for this object
    {
                                                ...
                                                // attribute
      {
        messages
            .add(new String(
                "You must make a different attribute the key in order to remove this one as the key."));
      } else // this is the only attribute
      {
        messages
            .add(new String(
                "You cannot remove this attribute as the key since it is the only attribute."));
      }
    } else if ((objectInFocus.getNumAttributes() == 0)
        && (((String) keyList.getSelectedItem()) == "No")) // this is a new
                                                           // attribute being
    // created and it is also the first attribute being created for the object
    // type in focus
    {
      messages
          .add(new String(
              "You must have exactly one attribute per object type defined as the key."));
    }
    ...
    if (selectedType == AttributeTypes.DOUBLE) // type = double
    {
      ...
      if ((minDigitsInput != null) && (minDigitsInput.length() > 0)) // field is
                                                                     // not
                                                                     // blank
      {
        try {
          ...
          messages
              .add(new String(
                  "Minimum number of fraction digits must be a valid integer number between 0 and 16"));
        }
      }

      // Check maxDigits input:
      if ((maxDigitsInput != null) && (maxDigitsInput.length() > 0)) // field is
                                                                     // not
                                                                     // blank
      {
        try {
          ...
          messages
              .add(new String(
                  "Maximum number of fraction digits must be a valid integer number between 0 and 16"));
        }
      }
    }
  ...
  private String maxGreaterThanOrEqualToMin() // returns a string with the error
                                              // message if this condition is
                                              // not met, null otherwise
  {
    String minValInput = minValTextField.getText();
    String maxValInput = maxValTextField.getText();
  ...
  private String maxDigitsGreaterThanOrEqualToMinDigits() // returns a string
                                                          // with the error
                                                          // message if this
                                                          // condition is not
                                                          // met, null
  // otherwise
  {
    ...
    if ((minDigitsInput != null) && (minDigitsInput.length() > 0)
        && (maxDigitsInput != null) && (maxDigitsInput.length() > 0)) // neither
    // max nor min fields are blank
    {
      ...
      if (selectedType == AttributeTypes.DOUBLE) {
        try {
          ...
          {
            return (new String(
                "Maximum number of fraction digits must be greater than or equal to minimum value"));
          }
        } catch (NumberFormatException e) {
          System.out.println(e.getMessage());
          // since this has already been checked with call to inputValid(), this
  ...
  private boolean attributeNameIsUnique() // returns true if it is unique, false
                                          // otherwise, and also takes care of
                                          // asking the user
  // if they want to overwrite it (and overwriting it, if desired)
  {
    String nameInput = nameTextField.getText();
    if ((attribute == null)
        || ((attribute != null) && (attribute.getName().equals(nameInput) == false))) // only
                                                                                      // perform
                                                                                      // this
                                                                                      // check
                                                                                      // if
    // this is a newly created attribute, or it is an edited attribute and the
      ...
      for (int i = 0; i < existingAttributes.size(); i++) {
        ...
        // this object defined with the same name)
        {
          int choice = JOptionPane
              .showConfirmDialog(
                  null,
                  ("Previously defined " + tempAttr.getName() + " attribute will be overwritten. Continue?"),
          ...
          if (choice == JOptionPane.YES_OPTION) {
                                   ...
                                   // new one
            {
              ...
              // was the key previously, but the user has just changed it to not
              // be the key, hence there would be no key
              // for this object
              {
              ...
              } else // everything is okay!
              {
                ...
                if (((String) keyList.getSelectedItem()) == "Yes") // this newly
                                                                   // added/edited
                                                                   // attribute
                                                                   // is key
                {
                  Attribute key = objectInFocus.getKey(); // get the current key
                  if (keyAttName.equalsIgnoreCase(nameInput) == false) // newly
                                                                       // added/edited
                                                                       // attribute
                                                                       // was
                                                                       // not
                ...
                if (objectInFocus.getAllAttributes().size() == 1) // only one
                                                                  // attribute
                {
                  ((Attribute) (objectInFocus.getAllAttributes().elementAt(0)))
                      .setKey(true); // make sure the one attribute is the key
                }
              }
            } else // this is a new attribute, not one being edited
            {
                                                                 ...
                                                                 // key
              {
                if (objectInFocus.hasKey()) {
                  Attribute key = objectInFocus.getKey(); // get the current key
                  if (key.getName().equalsIgnoreCase(nameInput) == false) // newly
                                                                          // added/edited
                                                                          // attribute
                                                                          // was
                                                                          // not
                  ...
                  && (tempAttr.isKey())) // this newly added attribute is
              // not the key, but the one it is supposed to be replacing was the
              // key
              {
  ...
  private void addAttribute() // creates a new attribute from the existing info
                              // in the form and adds it to the object in focus.
  // Note: inputValid(), maxGreaterThanOrEqualToMin(), and
  // attributeNameIsUnique() should all be called before calling this
  // function to ensure that you're adding a valid attribute
  {
    ...
    if (((String) keyList.getSelectedItem()) == "Yes") {
      ...
      if (objectInFocus.hasKey()) {
        ...
        if (keyAttr.getName().equals(nameTextField.getText()) == false) // newly
                                                                        // added/edited
                                                                        // attribute
                                                                        // was
                                                                        // not
        ...
        || (selectedType == AttributeTypes.INTEGER)) // numerical attribute
    {
      ...
      if (selectedType == AttributeTypes.DOUBLE) {
        try {
          ...
          } else if ((minValInput != null) && (minValInput.length() > 0)) // only
                                                                          // max
                                                                          // val
                                                                          // field
                                                                          // is
                                                                          // blank
          {
            double minVal = Double.parseDouble(minValInput);
            min = new Double(minVal);
          } else if ((maxValInput != null) && (maxValInput.length() > 0)) // only
                                                                          // min
                                                                          // val
                                                                          // field
                                                                          // is
          ...
          } else if ((minDigitsInput != null) && (minDigitsInput.length() > 0)) // only
                                                                                // max
                                                                                // field
                                                                                // is
                                                                                // blank
          ...
          } else if ((maxDigitsInput != null) && (maxDigitsInput.length() > 0)) // only
                                                                                // min
                                                                                // field
                                                                                // is
                                                                                // blank
  ...
  private void editAttribute() // changes the attribute being edited to reflect
                               // the changes made in this form.
  // Note: inputValid(), maxGreaterThanOrEqualToMin(), and
  // attributeNameIsUnique() should all be called before calling this
        ...
        || (selectedType == AttributeTypes.INTEGER)) // numerical attribute
    {
      if ((attribute.getType() == AttributeTypes.STRING)
          || (attribute.getType() == AttributeTypes.BOOLEAN)) // attribute is
                                                              // changing from
      // a non-numerical attribute to a numerical one, so needs to be created
      // new
      {
        int position = objectInFocus.removeAttribute(attribute.getName()); // remove
                                                                           // old
                                                                           // non-numerical
                                                                           // attribute
        objectInFocus.addAttribute(new NumericalAttribute(
            (NonNumericalAttribute) attribute, selectedType), position); // add
                                                                         // a
                                                                         // new
                                                                         // numerical
        // attribute
        attribute = objectInFocus.getAttribute(nameTextField.getText()); // re-focus
                                                                         // attribute
                                                                         // to
                                                                         // this
                                                                         // newly
      ...
      if (selectedType == AttributeTypes.DOUBLE) {
        try {
          ...
          // val nor max val field is blank
          {
            ...
            ((NumericalAttribute) attribute).setMinValue(new Double(Double
                .parseDouble(minValInput)));
            ((NumericalAttribute) attribute).setMaxValue(new Double(Double
                .parseDouble(maxValInput)));
          } else if ((minValInput != null) && (minValInput.length() > 0)) // only
                                                                          // max
                                                                          // val
                                                                          // field
          ...
          // neither min nor max field is blank
          {
            ...
            ((NumericalAttribute) attribute)
                .setMinNumFractionDigits(new Integer(Integer
                    .parseInt(minDigitsInput)));
            ((NumericalAttribute) attribute)
                .setMaxNumFractionDigits(new Integer(Integer
                    .parseInt(maxDigitsInput)));
          } else if ((minDigitsInput != null) && (minDigitsInput.length() > 0)) // only
                                                                                // max
                                                                                // field
                                                                                // is
        ...
        try {
          ...
          {
            // Set min and max vals to new vals:
            ((NumericalAttribute) attribute).setMinValue(new Integer(Integer
                .parseInt(minValInput)));
            ((NumericalAttribute) attribute).setMaxValue(new Integer(Integer
                .parseInt(maxValInput)));
          } else if ((minValInput != null) && (minValInput.length() > 0)) // only
                                                                          // max
                                                                          // val
                                                                          // field
    ...
    } else // type is String or Boolean, hence a non-numerical attribute
    {
      ...
      // to a non-numerical one, so needs to be created new
      {
        ...
        objectInFocus.addAttribute(new NonNumericalAttribute(
            (NumericalAttribute) attribute, selectedType), position); // add a
                                                                      // new
                                                                      // non-numerical
        // attribute
        attribute = objectInFocus.getAttribute(nameTextField.getText()); // re-focus
