/* This class is responsible for generating all of the code for the different At-A-Glance Frames in the GUI */
...
public class AtAGlanceFramesGenerator
{
	...
	{
		// generate file:
		File frameFile = new File(directory, ("simse\\gui\\" + type + "sAtAGlanceFrame.java"));
		if(frameFile.exists())
		{
		...
		try
		{
			...
			writer.write("public class " + type + "sAtAGlanceFrame extends JFrame implements MouseListener, ActionListener");
			writer.write(NEWLINE);
			...
			for(int i=0; i<types.size(); i++)
			{
				...
				writer.write("private JTable " + tempType.getName().toLowerCase() + "Table;");
				writer.write(NEWLINE);
				writer.write("private " + getUpperCaseLeading(tempType.getName()) + "TableModel " + tempType.getName().toLowerCase()
					+ "Model;");
				writer.write(NEWLINE);
				writer.write("private JPanel " + tempType.getName().toLowerCase() + "TitlePane;");
				writer.write(NEWLINE);
			}
			...
			{
				SimSEObjectType tempType = (SimSEObjectType)types.elementAt(i);
				writer.write(tempType.getName().toLowerCase() + "Model = new " + getUpperCaseLeading(tempType.getName()) + "TableModel(s);");
				writer.write(NEWLINE);
				writer.write(tempType.getName().toLowerCase() + "Table = new JTable(" + tempType.getName().toLowerCase() + "Model);");
				writer.write(NEWLINE);
				writer.write(tempType.getName().toLowerCase() + "Table.setColumnSelectionAllowed(false);");
				writer.write(NEWLINE);
				writer.write(tempType.getName().toLowerCase() + "Table.setRowSelectionAllowed(false);");
				writer.write(NEWLINE);
				...
				writer.write(OPEN_BRACK);
				writer.write(NEWLINE);
				writer.write(tempType.getName().toLowerCase() + "Table.getColumnModel().getColumn(i).setMinWidth(0);");
				writer.write(NEWLINE);
			...
			writer.write(NEWLINE);
			writer.write("mainPane.setLayout(new BoxLayout(mainPane, BoxLayout.Y_AXIS));");
			writer.write(NEWLINE);
			writer.write(NEWLINE);
			...
			for(int i=0; i<types.size(); i++)
			{
				...
				writer.write("if(me.getComponent().equals(" + tempType.getName().toLowerCase() + "Table)) // correct table");
				writer.write(NEWLINE);
				writer.write(OPEN_BRACK);
				writer.write(NEWLINE);
				writer.write("createPopupMenu(" + tempType.getName().toLowerCase() + "Table, p);");
				writer.write(NEWLINE);
				writer.write(CLOSED_BRACK);
				writer.write(NEWLINE);
			}
			...
			writer.write(OPEN_BRACK);
			writer.write(NEWLINE);
			writer.write("selectedTable.getColumnModel().getColumn(realColumnIndex).setMaxWidth(0);");
			writer.write(NEWLINE);
			writer.write(CLOSED_BRACK);
			writer.write(NEWLINE);
			writer.write(CLOSED_BRACK);
			...
			for(int i=0; i<types.size(); i++)
			{
				...
				writer.write("if(model instanceof " + getUpperCaseLeading(tempType.getName()) + "TableModel)");
				writer.write(NEWLINE);
				writer.write(OPEN_BRACK);
				writer.write(NEWLINE);
				writer.write("column = selectedTable.getColumnModel().getColumn(((" + getUpperCaseLeading(tempType.getName())
			...
			writer.write(NEWLINE);
			writer.write("column.setPreferredWidth(selectedTable.getWidth() / (selectedTable.getColumnCount() - getAllHiddenColumnIndices(selectedTable).size() + 1));");
			writer.write(NEWLINE);
			writer.write(CLOSED_BRACK);
			writer.write(NEWLINE);
			writer.write(CLOSED_BRACK);
  	  ...
  	  writer.write("int colIndex = table.columnAtPoint(p);");
			writer.write(NEWLINE);
			writer.write("realColumnIndex = table.convertColumnIndexToModel(colIndex);");
			writer.write(NEWLINE);
			writer.write(NEWLINE);
			writer.write("Vector hiddenCols = getAllHiddenColumnIndices(table);");
			writer.write(NEWLINE);
			writer.write(NEWLINE);
			writer.write("if((realColumnIndex >= 0) || (hiddenCols.size() > 0)) // user clicked on a column and/or there is at least one hidden column");
			writer.write(NEWLINE);
			writer.write(OPEN_BRACK);
			writer.write(NEWLINE);
			...
			writer.write(NEWLINE);
			writer.write(NEWLINE);
			writer.write("if(hiddenCols.size() > 0) // there is at least one hidden column");
			writer.write(NEWLINE);
			writer.write(OPEN_BRACK);
			writer.write(NEWLINE);
			...
			writer.write(OPEN_BRACK);
			writer.write(NEWLINE);
			writer.write("int index = ((Integer)hiddenCols.elementAt(i)).intValue();");
			writer.write(NEWLINE);
			writer.write("JMenuItem tempItem = new JMenuItem(table.getColumnName(index));");
			writer.write(NEWLINE);
			...
			writer.write(CLOSED_BRACK);
			writer.write(NEWLINE);
			writer.write("if(popup.getComponents().length > 0) // already has the hide menu item");
			writer.write(NEWLINE);
			writer.write(OPEN_BRACK);
			writer.write(NEWLINE);
			...
			for(int i=0; i<types.size(); i++)
			{
				...
				writer.write(NEWLINE);
				writer.write("height += " + tempType.getName().toLowerCase() + "TitlePane.getSize().getHeight();");
				writer.write(NEWLINE);
			}
			writer.write(NEWLINE);
			writer.write("mainPane.setPreferredSize(new Dimension((int)(mainPane.getSize().getWidth()), (int)height));");
			writer.write(NEWLINE);
			writer.write("pack();");
			writer.write(NEWLINE);
			writer.write("validate();");
		...
		{
			JOptionPane.showMessageDialog(null, ("Error writing file " + frameFile.getPath() + ": " + e.toString()), "File IO Error",
