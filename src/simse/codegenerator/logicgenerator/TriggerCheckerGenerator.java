/* This class is responsible for generating all of the code for the trigger checker component in SimSE*/
...
public class TriggerCheckerGenerator
{
	...
	private Vector vectors; // for keeping track of which vectors are being used in generated code so that you don't generate the
		// same ones more than once -- e.g., Vector programmers 			// state.getEmployeeStateRepository().getProgrammerStateRepository().getAll() will be generated more than once if you
	...
	public void generate()
	{
		try
		{
			...
			// update function:
			writer.write("public void update(boolean updateUserTrigsOnly, JFrame gui)");
			writer.write(NEWLINE);
			writer.write(OPEN_BRACK);
			writer.write(NEWLINE);
			int counter = 0;
			...
			{
				generateTriggerChecker((ActionTypeTrigger)prioritizedTriggers.elementAt(i), counter);
				counter++;
			}
		...
		{
			JOptionPane.showMessageDialog(null, ("Error writing file " + trigFile.getPath() + ": " + e.toString()), "File IO Error",
	...
	private void generateTriggerChecker(ActionTypeTrigger outerTrig, int counter)
	{
		try
		{
			...
			for(int j=0; j<triggers.size(); j++)
			{
				...
				writer.write("Vector " + trig.getParticipant().getName().toLowerCase() + "s" + counter + " = new Vector();");
				writer.write(NEWLINE);
				...
				for(int k=0; k<constraints.size(); k++)
				{
							...
							//only be in one of these actions in this role at a time
					{
						...
						writer.write(OPEN_BRACK);
						writer.write(NEWLINE);
						writer.write(getUpperCaseLeading(action.getName()) + "Action b = (" + getUpperCaseLeading(action.getName())
			...
			else if((outerTrig instanceof AutonomousActionTypeTrigger) || (outerTrig instanceof
				RandomActionTypeTrigger))
			{
				writer.write("while(");
			}
			...
			if(outerTrig instanceof UserActionTypeTrigger)
			{
				...
				for(int k=0; k<parts.size(); k++)
				{
					...
					if(part.getSimSEObjectTypeType() == SimSEObjectTypeTypes.EMPLOYEE)
					{
						writer.write("for(int j=0; j<" + part.getName().toLowerCase() + "s" + counter + ".size(); j++)");
						writer.write(NEWLINE);
						writer.write(OPEN_BRACK);
						writer.write(NEWLINE);
						writer.write("Employee z = (Employee)" + part.getName().toLowerCase() + "s" + counter + ".elementAt(j);");
						writer.write(NEWLINE);
				...
				RandomActionTypeTrigger))
			{
				writer.write(getUpperCaseLeading(action.getName()) + "Action a = new " + getUpperCaseLeading(action.getName())
					+ "Action();");
				writer.write(NEWLINE);
				for(int k=0; k<parts.size(); k++)
				{
					...
					writer.write(OPEN_BRACK);
					writer.write(NEWLINE);
					writer.write(getUpperCaseLeading(SimSEObjectTypeTypes.getText(part.getSimSEObjectTypeType())) + " a" + k + " = ("
						+ getUpperCaseLeading(SimSEObjectTypeTypes.getText(part.getSimSEObjectTypeType())) + ")"
			...
			if(outerTrig instanceof UserActionTypeTrigger)
			{
				...
				writer.write(OPEN_BRACK);
				writer.write(NEWLINE);
				writer.write("((Employee)f" + cnt + ".elementAt(i)).removeMenuItem(\"JOIN " + ((UserActionTypeTrigger)outerTrig).getMenuText()
					+ "\");");
				writer.write(NEWLINE);
				writer.write(CLOSED_BRACK);
				writer.write(NEWLINE);
				...
				writer.write(OPEN_BRACK);
				writer.write(NEWLINE);
				writer.write(getUpperCaseLeading(action.getName()) + "Action a" +  cnt + " = (" + getUpperCaseLeading(action.getName()) + "Action)a" + cnt + "s.elementAt(i);");
				writer.write(NEWLINE);
				...
				for(int j=0; j<parts.size(); j++)
				{
						...
						// play
					{
						...
						for(int k=0; k<types.size(); k++)
						{
							...
							writer.write("(b" + cnt + " instanceof " + getUpperCaseLeading(tempType.getName()) + ")");
						}
						writer.write(") && (a" + cnt + ".getAll" + part.getName() + "s().contains(b" + cnt + ") == false))");
						writer.write(NEWLINE);
						writer.write(OPEN_BRACK);
						writer.write(NEWLINE);
						...
						//{
							...
							writer.write(NEWLINE);
						//}
						writer.write("if(z" + cnt + " && (b" + cnt + ".getMenu().contains(\"JOIN " + ((UserActionTypeTrigger)outerTrig).getMenuText() + "\") == false))");
						writer.write(NEWLINE);
						writer.write(OPEN_BRACK);
						writer.write(NEWLINE);
			...
			for(int j=0; j<trigs.size(); j++)
			{
				...
				else // priority >= 0
				{
					...
					else
					{
						...
						{
							ActionTypeTrigger tempA = (ActionTypeTrigger)prioritizedTriggers.elementAt(k);
							if(priority <= tempA.getPriority())
							{
								prioritizedTriggers.insertElementAt(tempTrig, k); // insert the trigger
								break;
							}
							else if(k == (prioritizedTriggers.size() - 1)) // on the last element
							{
								prioritizedTriggers.add(tempTrig); // add the trigger to the end of the list
								break;
							}
						}
					}
