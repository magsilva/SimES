/* This class is responsible for generating all of the code for the logic's NonEmployeeParticipantSelectionDialog component */
...
public class NonEmployeeParticipantSelectionDialogGenerator implements CodeGeneratorConstants
{
	...
	*/
	...
	public NonEmployeeParticipantSelectionDialogGenerator(DefinedActionTypes acts, DefinedObjectTypes objs, File dir)
	{
		directory = dir;
		actTypes = acts;
		objTypes = objs;
	...
	public void generate()
	{
		try
		{
			psdFile = new File(directory, ("simse\\logic\\dialogs\\NonEmployeeParticipantSelectionDialog.java"));
			if(psdFile.exists())
			{
			...
			writer.write(NEWLINE);
			writer.write("public class NonEmployeeParticipantSelectionDialog extends JDialog implements ActionListener");
			writer.write(NEWLINE);
			...
			writer.write(NEWLINE);
			// constructor:
			writer.write("public NonEmployeeParticipantSelectionDialog(JFrame owner, String pName, Vector parts, simse.adts.actions.Action act, State s)");
			writer.write(NEWLINE);
			writer.write(OPEN_BRACK);
			writer.write(NEWLINE);
			...
			for(int j=0; j<acts.size(); j++)
			{
				...
				{
					ActionTypeParticipant tempPart = (ActionTypeParticipant)participants.elementAt(k);
					if(tempPart.getSimSEObjectTypeType() != SimSEObjectTypeTypes.EMPLOYEE) // Non-Employee participant
					{
