	...
	*/
	...
	public void generate()
	{
		try
		{
			...
			writer.write("/* File generated by: simse.codegenerator.logicgenerator.dialoggenerator.ChooseRoleToPlayDialogGenerator */");
			...
			writer.write(NEWLINE);
			// constructor:
			writer.write("public ChooseRoleToPlayDialog(JFrame owner, Vector partNames, Employee e, simse.adts.actions.Action act, String menText, RuleExecutor re)");
			writer.write(NEWLINE);
			writer.write(OPEN_BRACK);
			writer.write(NEWLINE);
			...
			for(int i=0; i<userTrigActs.size(); i++)
			{
				...
				for(int j=0; j<allParts.size(); j++)
				{
					...
					if(part.getSimSEObjectTypeType() == SimSEObjectTypeTypes.EMPLOYEE) // employee participant
					{
						...
						// go through all trigger rules and execute rules that are to be executed on each join:
						...
						for(int k=0; k<rules.size(); k++)
						{
							...
							{
								writer.write("ruleExec.update(gui, RuleExecutor.UPDATE_ONE, " + "\"" + tempRule.getName() + "\", action);");
								writer.write(NEWLINE);
							}
						}
